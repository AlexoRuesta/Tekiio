...
function (runtime, libSearch, record, email, format) {
    ...
    function map(context) {
        try {
            ...
            const cuit = cols[0];
            // const tipoSujeto = cols[1];
            const descEstado = cols[3];
            // const descEstado = cols[3];
            // const fechaEmision = cols[4];
            const vigDesde = cols[9];
            const vigHasta = cols[10];
            const nroCertificado = cols[11];

            if(descEstado == 'Certificado emitido' && vigDesde != '-' && vigHasta != '-'){
                context.write({
                    key: cuit,
                        ...
                        try { 
                            record.submitFields.promise({
                                ...
                                values: {
                                    ...
                                    custentity_l54_porcentaje_excl : 100
                                },
                                options: {
    ...
    function getParameters() {
        try {
            ...
            parameters.input = JSON.parse(currScript.getParameter({ name: 'custscript_l54_pad_rg2681_mr_input'}));
    ...
    function enviarNotificacionFinal(estadisticas) {
        try {
            ...
            var subject = "Proceso Carga de Padrones RG2681 - Completado ";
