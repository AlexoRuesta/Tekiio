 ...
 */
 ...
 define(['N/format', 'L54/utilidades', 'N/search', 'N/runtime'],
    function (format, utilidades, search, runtime) {
        ...
        var proceso = 'afterVendorPaymentCreated';
        var tipoContSUSS;
        var tipoContGAN;
        var padronEspecial = new Array();
        var jurisdiccionEspecial = new Array();
        
        function afterVendorPaymentCreated(parameters) {
            ...
            try {
                    ...
                    var retencionHonorarios = script.getParameter('custscript_l54_cal_ret_pg_ret_li_gan_hon');
                    var jurisdiccionCordoba = script.getParameter('custscript_l54_cal_ret_pg_ret_li_jur_cor');
                    var paramAplicaEnLaProvincia = script.getParameter('custscript_l54_cal_ret_pg_apl_cal_en_pro');
                    var paramAplicaFueraProvincia = script.getParameter('custscript_l54_cal_ret_pg_apl_cal_fu_pro');
                    var paramNoAplicaProvincia = script.getParameter('custscript_l54_cal_ret_pg_no_apl_cal_ter');
                    var paramJurisdTucuman = script.getParameter('custscript_l54_cal_ret_pg_jurisd_tucuman');
                    ...
                    var informacionPago = parameters;
                    log.audit(proceso, 'Parametros recibidos informacionPago: ' + JSON.stringify(informacionPago));
                    ...
                    if (!isEmpty(informacionPago)) {
                        ...
                        if (!isEmpty(validacionCalculoRet) && validacionCalculoRet.error == false) {
                            let infoLineasJurisdiccionesIIBB = [];

                            var retencion_ganancias     = new Array(),
                                retencion_suss          = new Array(),
                                retencion_iva           = new Array();
                            
                            var total_retenciones       = 0.0,
                                imp_retencion_ganancias = 0.0,
                                imp_retencion_suss      = 0.0,
                            ...
                            var entity              = informacionPago.entity,
                                id_posting_period   = informacionPago.periodo,
                            ...
                            if (!isEmpty(resultadoDatosImp) && resultadoDatosImp.length > 0) {
                                var esAgenteRetencionGan    = resultadoDatosImp[0].esAgenteGanancias,
                                    esAgenteRetencionSUSS   = resultadoDatosImp[0].esAgenteSUSS,
                                    esAgenteRetencionIVA    = resultadoDatosImp[0].esAgenteIVA,
                                    esAgenteRetencionIIBB   = resultadoDatosImp[0].esAgenteIIBB,
                                    estadoExentoGan         = resultadoDatosImp[0].exentoGanancias,
                                    estadoExentoSUSS        = resultadoDatosImp[0].exentoSUSS,
                                    ...
                                    if (esAgenteRetencionIVA == false || (esAgenteRetencionIVA == true && !isEmpty(estadosExentos) && !isEmpty(estadoExentoIVA) && estadoExentoIVA > 0)) {
                                        if (esAgenteRetencionIIBB == false || (esAgenteRetencionIIBB == true && !isEmpty(estadosExentos) && !isEmpty(estadoExentoIIBB) && estadoExentoIIBB > 0)) {
                                            var billsPagarAux       = new Array(),
                                                billsPagar          = new Array(),
                                                contadorFacturas    = 0,
                                                sinJurisdiccionUnica= true,
                                                jurisdiccionesAgenteRetencion = null;

                                            for (var i = 0; i < informacionPago.facturas.length; i++) {
                                                
                                                    var id_vendorbill   = informacionPago.facturas[i].idVendorBill,
                                                    amount          = informacionPago.facturas[i].amount;
                                            ...
                                            // Función que contiene los netos de las facturas a nivel de líneas de la transacción y se extraen los datos de las lineas
                                            if (!esONG) {
                                                var resultsNetosVBLineas = obtener_arreglo_netos_vendorbill_lineas(entity, billsPagarAux, subsidiariaPago, sinJurisdiccionUnica, esONG);
                                            } else {
                                                var resultsNetosVBLineas = obtener_arreglo_netos_vendorbill_lineas_ong(entity, billsPagarAux, subsidiariaPago, sinJurisdiccionUnica, esONG);
                                            }
                                            
                                            
                                            // Obtengo el array de las jurisdicciones de entrega que fueron definidas en las facturas
                                            var jurisdiccionesEntregaFacturas = obtener_jurisdicciones_entrega_facturas(resultsNetosVBLineas);
                                            ...
                                            if (errorGeneral == false) {
                                                ...
                                                var codigoRetMGananciasConfigurado  = false,
                                                    codigoRetMIVAConfigurado        = false,
                                                    errorConfCodRetMGanancias       = false,
                                                    errorConfCodRetMIVA             = false;
                                                ...
                                                log.debug('CALCULARETENCIONES  - codigoRetencionMGanancias: ', 'CALCULARETENCIONES  - codigoRetencionMGanancias: ' + JSON.stringify(codigoRetencionMGanancias) + ', codigoRetencionMGanancias.length: ' + codigoRetencionMGanancias.length + ' ** TIEMPO' + new Date());
                                                ...
                                                log.debug('CALCULARETENCIONES - codigoRetencionMIVA', JSON.stringify(codigoRetencionMIVA) + ', codigoRetencionMIVA.length: ' + codigoRetencionMIVA.length + ' ** TIEMPO' + new Date());
                                                ...
                                                // Función que contiene los códigos de retención de las facturas a nivel general; además obtiene el importe bruto de cada transacción (factura compra, crédito factura compra)
                                                var resultsCodigosRetVB = obtener_arreglo_codigos_ret_vendorbill(entity, billsPagarAux, subsidiariaPago, esONG);
                                                
                                                // Función que contiene los códigos de retención de las facturas a nivel de líneas de la transacción; además obtiene el importe bruto de cada línea de la transacción
                                                if (!esONG) {
                                                    var resultsCodigosRetVBLineas = obtener_arreglo_codigos_ret_vendorbill_lineas(entity, billsPagarAux, subsidiariaPago, esONG);
                                                } else {
                                                    var resultsCodigosRetVBLineas = obtener_arreglo_codigos_ret_vendorbill_lineas_ong(entity, billsPagarAux, subsidiariaPago, esONG);
                                                }

                                                sinJurisdiccionUnica = false;
                                               
                                                var resultsTotalesBills         = obtenerTotalesTransacciones(entity, billsPagarAux, subsidiariaPago), // Se obtiene el total de las transacciones sin incluir líneas internas (para retenciones de Córdoba) 
                                                    resultsNetosVB              = obtener_arreglo_netos_vendorbill(entity, billsPagarAux, subsidiariaPago, sinJurisdiccionUnica, esONG), // Función que contiene los netos de las facturas a nivel general
                                                    resultsNetosJurisdiccion    = obtener_arreglo_netos_vendorbill_jurisdiccion(entity, billsPagarAux, subsidiariaPago, esONG),
                                                    resultsImpPerc              = obtener_arreglo_imp_perc(entity, billsPagarAux, subsidiariaPago),
                                                    resultsImpIVA               = obtener_arreglo_iva_vendorbill(entity, billsPagarAux, subsidiariaPago, esONG),
                                                    resultsPagPasFact           = calcularImpBrutosPagPasadosTotalesAcumulados(entity, billsPagarAux, subsidiariaPago, trandate),
                                                    arrayUnificadosImportesLineasFacturas = unificar_importes_lineas_facturas(resultsCodigosRetVBLineas, resultsNetosVBLineas),
                                                    billsPagadas                = unificar_info_facturas_lineas(billsPagar, arrayUnificadosImportesLineasFacturas); // Unificar importes brutos, importes netos, códigos de retenciones de las líneas de las facturas.
                                                ...
                                                var arrayIdVendorBill = [];
                                                var arrayIdLineVendorBill = [];

                                                for (var i = 1; i <= billsPagadas.length && pagoTotalFacturasM == true; i++) {
                                                    // Obtengo el identificador de la factura de proveedor aplicada
                                                    var esFacturaM = false;
                                                    var calcularSobreIVA = false;
                                                    var id_vendorbill = billsPagadas[i - 1].idVendorBill;
                                                    var lineNumber = billsPagadas[i - 1].lineNumber;
                                                    var esRetencionPorLinea = billsPagadas[i - 1].esRetencionPorLinea;
                                                    ...
                                                    var importe_neto_linea_factura_proveedor_total = billsPagadas[i - 1].importeNetoLinea;
                                                    let importeBrutoLineaAux = billsPagadas[i - 1].importeBrutoLinea;
                                                    var importe_bruto_facturas_iibb = 0.0;
                                                    var importe_bruto_facturas_aliados = 0.0;
                                                    var importe_total_factura_final = 0.0;
                                                    let esJurisdUtilizacion = false;
                                                    let esJurisdOrigen = false;
                                                    let esJurisdEntrega = false;
                                                    let esJurisdFact = false;
                                                    ...
                                                    var objCodigos = obtener_codigos_vendorbill(resultsCodigosRetVB, id_vendorbill, lineNumber, billsPagadas);
                                                    ...
                                                    log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 469 - importe_bruto_factura_proveedor_a_pagar antes de dividir por coeficiente: ' + importe_bruto_factura_proveedor_a_pagar + ' - coeficiente: ' + coeficiente + ' - ** TIEMPO' + new Date());
                                                    ...
                                                    log.debug('L54 - Calculo Retenciones', 'LINE 535 - importe_bruto_porcentaje: ' + importe_bruto_porcentaje);
                                                    
                                                    
                                                    /* INICIO - VERIFICO SI LA FACTURA QUE SE ESTA RECORRIENDO TIENE CODIGOS DE RETENCION DEFINIDOS A NIVEL DE LÍNEA; esto se hizo para cálculo de retenciones por línea. */

                                                    if (!utilidades.isEmpty(lineNumber) && esRetencionPorLinea) {

                                                        var cantidadDecimalesImporteButoTotalLinea = countDecimales(importe_bruto_porcentaje);
                                                        var cantidadDecimalesImporteNetoTotalLinea = countDecimales(importe_neto_linea_factura_proveedor_total);
                                                        var cantidadDecimalesImporteNetoFinalLinea = 2 + cantidadDecimalesImporteButoTotalLinea + cantidadDecimalesImporteNetoTotalLinea;
                                                        importe_neto_linea_factura_proveedor_a_pagar_aux = parseFloat((parseFloat(convertToInteger(parseFloat(importe_neto_linea_factura_proveedor_total, 10)) * convertToInteger(parseFloat(importe_bruto_porcentaje, 10)), 10) / parseFloat(Math.pow(10, cantidadDecimalesImporteNetoFinalLinea), 10)), 10);
                                                        importeBrutoLineaAux = parseFloat(parseFloat((parseFloat(importeBrutoLineaAux, 10) * parseFloat(importe_bruto_porcentaje, 10) / 100), 10) * parseFloat(tasa_cambio_pago, 10), 10);
                                                        
                                                        log.debug('L54 - Calculo Retenciones', 'LINE 537 - importe_neto_linea_factura_proveedor_a_pagar_aux: ' + importe_neto_linea_factura_proveedor_a_pagar_aux + ' / importeBrutoLineaAux: ' + importeBrutoLineaAux);
                                                        // Se obtiene el importe bruto y neto de la linea y se procede a multiplicar por el tipo de cambio.
                                                        importe_neto_linea_factura_proveedor_a_pagar = parseFloat(parseFloat(importe_neto_linea_factura_proveedor_a_pagar_aux, 10) * parseFloat(tasa_cambio_pago, 10), 10);
                                                        // importeBrutoLineaAux = parseFloat(parseFloat(importe_bruto_factura_proveedor_a_pagar, 10) * parseFloat(tasa_cambio_pago, 10), 10);

                                                        log.debug('L54 - Calculo Retenciones', 'LINE 539 - importe_neto_linea_factura_proveedor_a_pagar: ' + importe_neto_linea_factura_proveedor_a_pagar);

                                                        importe_neto_linea_factura_proveedor_a_pagar = (parseFloat(countDecimales(importe_neto_linea_factura_proveedor_a_pagar), 10) >= 10) ? parseFloat(parseFloat(importe_neto_linea_factura_proveedor_a_pagar, 10).toFixedOK(2), 10) : importe_neto_linea_factura_proveedor_a_pagar;
                                                        importeBrutoLineaAux = (parseFloat(countDecimales(importeBrutoLineaAux), 10) >= 10) ? parseFloat(parseFloat(importeBrutoLineaAux, 10).toFixedOK(2), 10) : importeBrutoLineaAux;
                                                        
                                                        importeNetoPagado += parseFloat(importe_neto_linea_factura_proveedor_a_pagar, 10);
                                                        importeBrutoPagado += parseFloat(importeBrutoLineaAux, 10);

                                                        log.debug('L54 - Calculo Retenciones', 'LINE 550 - importe_neto_linea_factura_proveedor_a_pagar: ' + importe_neto_linea_factura_proveedor_a_pagar + ' / importeBrutoLineaAux: ' + importeBrutoLineaAux);

                                                    }

                                                    /* FIN - VERIFICO SI LA FACTURA QUE SE ESTA RECORRIENDO TIENE CODIGOS DE RETENCION DEFINIDOS A NIVEL DE LÍNEA; esto se hizo para cálculo de retenciones por línea. */


                                                    /* INICIO PROCESAMIENTO DE TRANSACCIONES POR CABECERA O QUE APLICAN A RETENCIONES POR CABECERA / Esto debe ejecutarse sólo una vez por cada factura procesada */

                                                    // SI NO ES RETENCION POR LINEAS O SI ES LA PRIMERA LINEA DE LA FACTURA
                                                    if ((utilidades.isEmpty(lineNumber) && !esRetencionPorLinea) || (!utilidades.isEmpty(lineNumber) && (arrayIdVendorBill.indexOf(id_vendorbill) == -1) && esRetencionPorLinea)) {

                                                        importesGlobalesProcesados = true;
                                                        ...
                                                        log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 474 - importe_neto_factura_proveedor_a_pagar en moneda de la transacción: ' + importe_neto_factura_proveedor_a_pagar + ' - ** TIEMPO' + new Date());
                                                        ...
                                                        objInfoFacturasJurisdiccionEntrega = obtener_info_facturas_jurisdiccion_entrega(resultsNetosVB, id_vendorbill, importe_neto_factura_proveedor_a_pagar_ret_iibb);
                                                        ...
                                                        log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES  - IMPORTE TOTAL FACTURA: ' + importe_bruto_factura_proveedor + ', IMPORTE PAGO FACTURA: ' + importe_bruto_factura_proveedor_a_pagar + ' ** TIEMPO' + new Date());
                                                        log.debug('L54 - Calculo Retenciones', 'LINE 518 - importe_neto_factura_proveedor_a_pagar_total: ' + importe_neto_factura_proveedor_a_pagar_total);
                                                        ...
                                                        log.debug('L54 - Calculo Retenciones', 'LINE 526 - importe_neto_factura_proveedor_a_pagar_total: ' + importe_neto_factura_proveedor_a_pagar_total);
                                                        ...
                                                        log.audit("Governance Monitoring", "LINE 674 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                        ...
                                                        importePercepcionesPagoParcial = parseFloat(importePercepcionesPagoAux, 10) * parseFloat(porcentajePago, 10);
                                                    ...
                                                    /* FIN PROCESAMIENTO DE TRANSACCIONES POR CABECERA O QUE APLICAN A RETENCIONES POR CABECERA */

                                                    log.audit("Governance Monitoring", "LINE 688 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());

                                                    if ((!utilidades.isEmpty(codigo_retencion_ganancias)) ||
                                                        (!utilidades.isEmpty(codigo_retencion_suss)) ||
                                                        (!utilidades.isEmpty(codigo_retencion_iva)) ||
                                                        (!utilidades.isEmpty(objCodigos.jurisdUtilizacion)) ||
                                                        (!utilidades.isEmpty(objCodigos.jurisdOrigen)) ||
                                                        (!utilidades.isEmpty(objCodigos.jurisdiccionEntregaID)) ||
                                                        (!utilidades.isEmpty(objCodigos.jurisdFacturacion))) {
                                                        ...
                                                        if (!isEmpty(codigo_retencion_ganancias)) {
                                                            ...
                                                            if (!isEmpty(resultGanRetManual) && resultGanRetManual.length > 0) {
                                                                ...
                                                                if (!utilidades.isEmpty(lineNumber) && esRetencionPorLinea) {
                                                                    importe_neto_factura_proveedor_a_pagar = parseFloat(importe_neto_linea_factura_proveedor_a_pagar, 10);
                                                                }
                                                                ...
                                                                if (esFacturaM == true && parseFloat(importe_neto_factura_proveedor_a_pagar, 10) >= 1000) {
                                                                    if (codigoRetMGananciasConfigurado == true) {
                                                                        retencion_ganancias = agregarCodigoRetencion(retencion_ganancias, codigoRetencionMGanancias[0].codigo, importe_neto_factura_proveedor_a_pagar, esFacturaM, importe_neto_factura_proveedor, calcularSobreIVA, nombre_retencion_ganancias, esRetManual);
                                                                    }
                                                                    else {
                                                        ...
                                                        log.audit("Governance Monitoring", "LINE 726 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                        ...
                                                        log.audit("Governance Monitoring", "LINE 748 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());

                                                        if (!isEmpty(codigo_retencion_iva)) {
                                                            var validateSobreIva = calcularSobreIVA ? arrayIdVendorBill.indexOf(id_vendorbill) == -1 : arrayIdLineVendorBill.indexOf(id_vendorbill + '//' + lineNumber) == -1;
                                                        ...
                                                        if (!isEmpty(objCodigos) && !isEmpty(objCodigos.jurisdUtilizacion)) {
                                                            ...
                                                            infoLineasJurisdiccionesIIBB = agregarJurisdiccionesIIBB(infoLineasJurisdiccionesIIBB, objCodigos.jurisdUtilizacion, importe_neto_linea_factura_proveedor_a_pagar, objCodigos.nombreJurisdUtilizacion, esJurisdUtilizacion, esJurisdOrigen, esJurisdEntrega, importeBrutoLineaAux, lineNumber, esJurisdFact, objCodigos, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia, id_vendorbill);
                                                        }
                                                        ...
                                                        if (!isEmpty(objCodigos) && !isEmpty(objCodigos.jurisdOrigen)) {
                                                            ...
                                                            infoLineasJurisdiccionesIIBB = agregarJurisdiccionesIIBB(infoLineasJurisdiccionesIIBB, objCodigos.jurisdOrigen, importe_neto_linea_factura_proveedor_a_pagar, objCodigos.nombreJurisdOrigen, esJurisdUtilizacion, esJurisdOrigen, esJurisdEntrega, importeBrutoLineaAux, lineNumber, esJurisdFact, objCodigos, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia, id_vendorbill);
                                                        }
                                                        ...
                                                        if (!isEmpty(objCodigos) && !isEmpty(objCodigos.jurisdiccionEntregaID)) {
                                                            ...
                                                            infoLineasJurisdiccionesIIBB = agregarJurisdiccionesIIBB(infoLineasJurisdiccionesIIBB, objCodigos.jurisdiccionEntregaID, importe_neto_linea_factura_proveedor_a_pagar, objCodigos.jurisdiccionEntregaNombre, esJurisdUtilizacion, esJurisdOrigen, esJurisdEntrega, importeBrutoLineaAux, lineNumber, esJurisdFact, objCodigos, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia, id_vendorbill);
                                                        }
                                                        ...
                                                        if (!isEmpty(objCodigos) && !isEmpty(objCodigos.jurisdFacturacion)) {
                                                            ...
                                                            /* SE DEBE UTILIZAR UNA VARIABLE PARA ALMACENAR LAS JURISDICCIONES DE DESTINO/ENTREGA */
                                                            infoLineasJurisdiccionesIIBB = agregarJurisdiccionesIIBB(infoLineasJurisdiccionesIIBB, objCodigos.jurisdFacturacion, importe_neto_linea_factura_proveedor_a_pagar, objCodigos.nombreJurisdFacturacion, esJurisdUtilizacion, esJurisdOrigen, esJurisdEntrega, importeBrutoLineaAux, lineNumber, esJurisdFact, objCodigos, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia, id_vendorbill);
                                                        }
                                                        
                                                        /* FIN - Agregar informacion de jurisdiccion de facturacion en las lineas */


                                                        log.audit("Governance Monitoring", "LINE 795 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                    }

                                                    arrayIdVendorBill.push(id_vendorbill);
                                                    arrayIdLineVendorBill.push(id_vendorbill + '//' + lineNumber)
                                                ...
                                                }//FIN del for (var i = 1; i <= billsPagadas.length && pagoTotalFacturasM==true; i++) -- El que recorre las facturas y suma los importes de las mismas.
                                                ...
                                                log.debug('LINE 961 - CALCULARETENCIONES - infoLineasJurisdiccionesIIBB: ', JSON.stringify(infoLineasJurisdiccionesIIBB) + ' ** TIEMPO' + new Date());

                                                log.debug(`L54 - Calculo Retenciones`, `LINE 924 - importeNetoPagado: ${importeNetoPagado} / importeBrutoPagado: ${importeBrutoPagado}`);

                                                // Nuevo - Si el Pago de la Factura M no es total Aviso.
                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - pagoTotalFacturasM: ' + pagoTotalFacturasM);
                                                ...
                                                if (pagoTotalFacturasM == true) {
                                                    ...
                                                    if (esAgenteRetencionIIBB == true) {
                                                        if (estaInscriptoRegimenIIBB == true) {
                                                            ...
                                                            if (!isEmpty(arrayJurisdicciones) && arrayJurisdicciones.length > 0) {
                                                                objRespuestaConfigJurisdiccionGeneral = getProveedorInscriptoRegimenIIBB(null, estadoExentoIIBB, arrayJurisdicciones, null, null, null, null, trandate, paramJurisdTucuman);
                                                            }
                                                            ...
                                                            log.debug('L54 - Calculo Retenciones', 'LINE 495 - mensajeJurisdiccionesNotValid: ' + mensajeJurisdiccionesNotValid);
                                                            log.debug('L54 - Calculo Retenciones', 'LINE 496 - objEstadoInscripcionJurIIBB: ' + JSON.stringify(objEstadoInscripcionJurIIBB.jurisdicciones));
                                                            ...
                                                            let objJurisdUnificado = unificarJurisdPorLineasInscripcion(objEstadoInscripcionJurIIBB, infoLineasJurisdiccionesIIBB, importeNetoPagado, importeBrutoPagado, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia);
                                                            log.debug('L54 - Calculo Retenciones', 'LINE 507 - objJurisdUnificado: ' + JSON.stringify(objJurisdUnificado));
                                                    ...
                                                    log.debug('L54 - Calculo Retenciones', 'LINE 508 - codigosRetencionIIBB: ' + JSON.stringify(codigosRetencionIIBB));
                                                    ...
                                                    if (calcularRetIIBB == false || (calcularRetIIBB == true && codigosRetencionIIBB != null && codigosRetencionIIBB.error == false)) {
                                                        ...
                                                        var arrayCodigosRetencion = [];
                                                        var arrayDetallesRetencion = [];
                                                        var arrayCodigosRetencionIIBB = [];
                                                        var arrayDetallesRetencionIIBB = [];
                                                        var esRetIIBB = false;
                                                        var arrayRetencionesMensuales = [];
                                                        var arrayRetencionesAnuales = [];
                                                        var arrayImpRetenidosAnteriores = [];
                                                        var arrayImpRetenidosAnterioresAnuales = [];
                                                        var esAnualizada = false;

                                                        // INICIO - Recorrido de arrays de códigos de retención de todos los tipos de retención para hacer dos únicos SS de obtención de detalles de códigos de retención y otros SS para mejorar consumo de unidades
                                                        if (esAgenteRetencionGan) {
                                                            if (estaInscriptoRegimenGan) {
                                                                ...
                                                                for (var i = 0; i < retencion_ganancias.length; i++) {
                                                                    ...
                                                                    if (esGanMonotributo) {
                                                                        arrayRetencionesAnuales.push(retencion_ganancias[i].codigo);
                                                                    } else {
                                                                        arrayRetencionesMensuales.push(retencion_ganancias[i].codigo);
                                                                    }

                                                                    arrayCodigosRetencion.push(parseInt(retencion_ganancias[i].codigo, 10));
                                                                }
                                                            ...
                                                            if (estaInscriptoRegimenSUSS) {
                                                                ...
                                                                for (var i = 0; i < retencion_suss.length; i++) {
                                                                    ...
                                                                    if (baseCalculoAnualizada) {
                                                                        arrayRetencionesAnuales.push(retencion_suss[i].codigo);
                                                                    } else {
                                                                        arrayRetencionesMensuales.push(retencion_suss[i].codigo);
                                                                    }

                                                                    arrayCodigosRetencion.push(parseInt(retencion_suss[i].codigo, 10));
                                                                }
                                                            ...
                                                            if (estaInscriptoRegimenIVA) {
                                                                ...
                                                                for (var i = 0; i < retencion_iva.length; i++) {
                                                                    if (!isEmpty(retencion_iva[i].codigo) && (!retencion_iva[i].calcularSobreIVA)) {
                                                                        arrayRetencionesMensuales.push(retencion_iva[i].codigo);
                                                                    }

                                                                    arrayCodigosRetencion.push(parseInt(retencion_iva[i].codigo, 10));
                                                                }
                                                        ...
                                                        if (esAgenteRetencionIIBB) {
                                                            if (estaInscriptoRegimenIIBB) {
                                                                for (var i = 0; codigosRetencionIIBB != null && codigosRetencionIIBB.infoRet != null && i < codigosRetencionIIBB.infoRet.length; i++) {
                                                                    arrayCodigosRetencionIIBB.push(parseInt(codigosRetencionIIBB.infoRet[i].codigo, 10));
                                                                }
                                                            }
                                                        }

                                                        if (!utilidades.isEmpty(arrayRetencionesMensuales) && arrayRetencionesMensuales.length > 0) {
                                                            arrayImpRetenidosAnteriores = getInfoRetencionCalcAnteriores(entity, id_posting_period, arrayRetencionesMensuales, subsidiariaPago, esAnualizada, trandate);
                                                        }
                                                        log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 964 - Despues de llamar a getInfoRetencionCalcAnteriores - arrayImpRetenidosAnteriores: ' + JSON.stringify(arrayImpRetenidosAnteriores) + ' ** TIEMPO' + new Date());

                                                        if (!utilidades.isEmpty(arrayRetencionesAnuales) && arrayRetencionesAnuales.length > 0) {
                                                            esAnualizada = true;
                                                            arrayImpRetenidosAnterioresAnuales = getInfoRetencionCalcAnteriores(entity, id_posting_period, arrayRetencionesAnuales, subsidiariaPago, esAnualizada, trandate);
                                                        }
                                                        log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 970 - Despues de llamar a getInfoRetencionCalcAnteriores - arrayImpRetenidosAnterioresAnuales: ' + JSON.stringify(arrayImpRetenidosAnterioresAnuales) + ' ** TIEMPO' + new Date());

                                                        // Busco los detalles de parametrización de retenciones de GAN, SUSS e IVA.
                                                        if (!utilidades.isEmpty(arrayCodigosRetencion) && arrayCodigosRetencion.length > 0) {
                                                            arrayDetallesRetencion = getParametrosMatriz(arrayCodigosRetencion, esRetIIBB);
                                                        }

                                                        // Busco los detalles de parametrización de retenciones de IIBB.
                                                        if (!utilidades.isEmpty(arrayCodigosRetencionIIBB) && arrayCodigosRetencionIIBB.length > 0) {
                                                            esRetIIBB = true;
                                                            arrayDetallesRetencionIIBB = getParametrosMatriz(arrayCodigosRetencionIIBB, esRetIIBB);
                                                        }
                                                        // FIN - Recorrido de arrays de códigos de retención de todos los tipos de retención para hacer dos únicos SS de obtención de detalles de códigos de retención y otros SS para mejorar consumo de unidades

                                                        if (esAgenteRetencionGan) { // Rete Gan
                                                                    ...
                                                                    if (!isEmpty(retencion_ganancias[i].esFacturaM) && retencion_ganancias[i].esFacturaM == true) {
                                                                        ...
                                                                        if (!isEmpty(retencion_ganancias[i].codigo)) {
                                                                            ...
                                                                            for (let l = 0; l < subListAcumulados.length; l++) {
                                                                                if(subListAcumulados[l].codigoRetención == retencion_ganancias[i].codigo){
                                                                                    impPagosPasadosTotal += parseFloat(subListAcumulados[l].impBaseCalculoAcumulada, 10);
                                                                                }
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 1254 - baseCalcPagosRetenidosGAN: ' + baseCalcPagosRetenidosGAN + ' - es anualizada: ' + esAnualizada + ' ** TIEMPO' + new Date());
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 1454 - diferenciaBaseCalculoPagado: ' + diferenciaBaseCalculoPagado + ' - baseCalcPagosRetenidosGAN: ' + baseCalcPagosRetenidosGAN + ' - minimoNoImponible: ' + minimoNoImponible + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                        
                                                                            var objRetencionGAN = getRetencion(entity, 'gan', retencion_ganancias[i].codigo, retencion_ganancias[i].base_calculo_retencion, id_posting_period, retencion_ganancias[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_ganancias[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                            log.debug('L54 - Calculo Retenciones', 'LINE 1281 - CALCULARETENCIONES  - objRetencionGAN: ' + JSON.stringify(objRetencionGAN) + ' ** TIEMPO' + new Date());
                                                                        ...
                                                                        if (!isEmpty(retencion_ganancias[i].codigo)) {
                                                                            ...
                                                                            if (esGanMonotributo) {
                                                                                ...
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 1699 - diferenciaBaseCalculoPagado: ' + diferenciaBaseCalculoPagado + ' - baseCalcPagosRetenidosGAN: ' + baseCalcPagosRetenidosGAN + ' - minimoNoImponible: ' + minimoNoImponible + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                        
                                                                                // var objRetencionGAN = getRetencion(entity, 'gan', retencion_ganancias[i].codigo, retencion_ganancias[i].base_calculo_retencion, id_posting_period, retencion_ganancias[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_ganancias[i].nombreRetencion, baseCalcPagosRetenidosGAN, arrayDetallesRetencion);
                                                                                var objRetencionGAN = getRetencion(entity, 'gan', retencion_ganancias[i].codigo, retencion_ganancias[i].base_calculo_retencion, id_posting_period, retencion_ganancias[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_ganancias[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                                ...
                                                                                if (retencion_ganancias[i].esRetManual) {
                                                                                    ...
                                                                                    log.debug('L54 - Calculo Retenciones', 'LINE 1727 - CALCULARETENCIONES - LINE 1724 - baseCalcPagosRetenidosGAN: ' + baseCalcPagosRetenidosGAN + ' - es anualizada: ' + esAnualizada + ' ** TIEMPO' + new Date());
                                                                                    impPagosPasadosTotal = parseFloat(numberTruncTwoDec(impPagosPasadosTotal), 10);
                                                                                    baseCalcPagosRetenidosGAN = parseFloat(numberTruncTwoDec(baseCalcPagosRetenidosGAN), 10);
                                                                                    ...
                                                                                    log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 1943 - diferenciaBaseCalculoPagado: ' + diferenciaBaseCalculoPagado + ' - baseCalcPagosRetenidosGAN: ' + baseCalcPagosRetenidosGAN + ' - minimoNoImponible: ' + minimoNoImponible + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                        ...
                                                        if (esAgenteRetencionSUSS) { // rete suss
                                                            ...
                                                            if (estaInscriptoRegimenSUSS) {
                                                                ...
                                                                if (!isEmpty(retencion_suss) && retencion_suss.length > 0) {
                                                                    ...
                                                                    log.audit("Governance Monitoring", "LINE 2062 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                }
                                                                // JSALAZAR: 09/12/19 - FIN - MANEJO DE IMPORTES Y MEJORAS PERFOMANCE RET. SUSS
                                                        
                                                                for (var i = 0; i < retencion_suss.length; i++) {
                                                        
                                                                    log.audit("Governance Monitoring", "LINE 2068 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                    ...
                                                                    if (!isEmpty(retencion_suss[i].codigo)) {
                                                                        ...
                                                                        if (baseCalculoAnualizada) {
                                                                            ...
                                                                            for (let l = 0; l < subListAcumulados.length; l++) {
                                                                                if(subListAcumulados[l].codigoRetención == retencion_suss[i].codigo){
                                                                                    impPagosPasadosTotal += parseFloat(subListAcumulados[l].impBaseCalculoAcumulada, 10);
                                                                                } 
                                                                            }
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2215 - resultMinNoImponible: ' + JSON.stringify(resultMinNoImponible));
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2481 - retencion_suss[i].importe_factura_pagar: ' + retencion_suss[i].importe_factura_pagar + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - minimoNoImponible: ' + minimoNoImponible + ' - ** TIEMPO' + new Date());
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES '+ ' - Importe Factura: ' + retencion_suss[i].importe_factura_pagar + ' - Tipo Cambio: ' + tasa_cambio_pago + ' Importe Retenido Anterior : ' + retencion_suss[i].imp_retenido_anterior + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                        
                                                                            var diferenciaBaseCalculoPagado = Math.abs(impPagosPasadosTotal - (baseCalcPagosRetenidosSUSS + parseFloat(minimoNoImponible, 10)));
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2490 - diferenciaBaseCalculoPagado: ' + diferenciaBaseCalculoPagado + ' - baseCalcPagosRetenidosSUSS: ' + baseCalcPagosRetenidosSUSS + ' - minimoNoImponible: ' + minimoNoImponible + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                        
                                                                            
                                                                            // var objRetencionSUSS = getRetencion(entity, 'suss', retencion_suss[i].codigo, retencion_suss[i].base_calculo_retencion, id_posting_period, retencion_suss[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_suss[i].nombreRetencion, baseCalcPagosRetenidosSUSS, arrayDetallesRetencion);
                                                                            var objRetencionSUSS = getRetencion(entity, 'suss', retencion_suss[i].codigo, retencion_suss[i].base_calculo_retencion, id_posting_period, retencion_suss[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_suss[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                            ...
                                                                            if (retencion_suss[i].esRetManual) {
                                                                                ...
                                                                                var baseCalcPagosRetenidosSUSS = 0.0;
                                                                                if (!isEmpty(objImporteRetenidoAnterior) && objImporteRetenidoAnterior.length > 0) {
                                                                                    baseCalcPagosRetenidosSUSS = parseFloat(objImporteRetenidoAnterior[0].importe, 10); // flagg
                                                                                }
                                                                                ...
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2709 - retencion_suss[i].importe_factura_pagar: ' + retencion_suss[i].importe_factura_pagar + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - minimoNoImponible: ' + minimoNoImponible + ' - ** TIEMPO' + new Date());
                                                                                ...
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2718 - diferenciaBaseCalculoPagado: ' + diferenciaBaseCalculoPagado + ' - baseCalcPagosRetenidosSUSS: ' + baseCalcPagosRetenidosSUSS + ' - minimoNoImponible: ' + minimoNoImponible + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                                                var objRetencionSUSS = getRetencion(entity, 'suss', retencion_suss[i].codigo, retencion_suss[i].base_calculo_retencion, id_posting_period, retencion_suss[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_suss[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - objRetencionSUSS: ' + JSON.stringify(objRetencionSUSS) + ' ** TIEMPO' + new Date());
                                                                                retencion_suss[i].importe_retencion = objRetencionSUSS.importeRetencion;
                                                                                retencion_suss[i].importe_retencion = parseFloat(retencion_suss[i].importe_retencion, 10); // En Moneda Base
                                                                        ...
                                                                        respuestaRetenciones.detalleAcumulados.push(extraerAcumuladoPorJurisdiccion(retencion_suss[i], entity, subsidiariaPago, id_posting_period, tasa_cambio_pago));
                                                                    ...
                                                                    log.audit("Governance Monitoring", "LINE 2514 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                }
                                                            }
                                                        ...
                                                        if (esAgenteRetencionIVA) { // rete iva
                                                            ...
                                                            if (estaInscriptoRegimenIVA) {
                                                                ...
                                                                if (!isEmpty(retencion_iva) && retencion_iva.length > 0) {
                                                                    ...
                                                                    log.audit("Governance Monitoring", "LINE 2548 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                        
                                                                    if (!utilidades.isEmpty(arrayCodRetIVA) && arrayCodRetIVA.length > 0) {
                                                                        var subListAcumulados = obtenerAcumSinJurisdicciones(entity, id_posting_period, subsidiariaPago, arrayCodRetIVA);    
                                                                    }
                                                        
                                                                    log.audit("Governance Monitoring", "LINE 2608 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                }
                                                                // JSALAZAR: 09/12/19 - FIN - MANEJO DE IMPORTES Y MEJORAS PERFOMANCE RET. IVA
                                                        
                                                                for (var i = 0; i < retencion_iva.length; i++) {
                                                        
                                                                    log.audit("Governance Monitoring", "LINE 2614 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                    var considerarImportesRetAnterior = true;
                                                                    ...
                                                                    if (!isEmpty(retencion_iva[i].esFacturaM) && retencion_iva[i].esFacturaM == true) {
                                                                        ...
                                                                        if (!isEmpty(retencion_iva[i].codigo)) {
                                                                            ...
                                                                            for (let l = 0; l < subListAcumulados.length; l++) {
                                                                                if(subListAcumulados[l].codigoRetención == retencion_iva[i].codigo){
                                                                                    impPagosPasadosTotal += parseFloat(subListAcumulados[l].impBaseCalculoAcumulada, 10);
                                                                                }
                                                                            }
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2970 - retencion_iva[i].importe_factura_pagar: ' + retencion_iva[i].importe_factura_pagar + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - minimoNoImponible: ' + minimoNoImponible + ' - ** TIEMPO' + new Date());
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'LINE 2973 - CALCULARETENCIONES - retencion_iva[i].base_calculo_retencion: ' + retencion_iva[i].base_calculo_retencion + ' ** TIEMPO' + new Date());
                                                                            ...
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - LINE 2978 - diferenciaBaseCalculoPagado: ' + diferenciaBaseCalculoPagado + ' - baseCalcPagosRetenidosIVA: ' + baseCalcPagosRetenidosIVA + ' - minimoNoImponible: ' + minimoNoImponible + ' - impPagosPasadosTotal: ' + impPagosPasadosTotal + ' - ** TIEMPO' + new Date());
                                                                            var objRetencionIVA = getRetencion(entity, 'iva', retencion_iva[i].codigo, retencion_iva[i].base_calculo_retencion, id_posting_period, retencion_iva[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_iva[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                        ...
                                                                        if (!isEmpty(retencion_iva[i].codigo) && !isEmpty(retencion_iva[i].calcularSobreIVA) && (retencion_iva[i].calcularSobreIVA)) {
                                                                            ...
                                                                            log.audit("Governance Monitoring", "LINE 2902 - getRetencion - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                            var objRetencionIVA = getRetencion(entity, 'iva', retencion_iva[i].codigo, retencion_iva[i].base_calculo_retencion, id_posting_period, retencion_iva[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_iva[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                            log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - objRetencionIVA: ' + JSON.stringify(objRetencionIVA) + ' ** TIEMPO' + new Date());
                                                                            retencion_iva[i].importe_retencion = objRetencionIVA.importeRetencion;
                                                                            retencion_iva[i].importe_retencion = parseFloat(retencion_iva[i].importe_retencion, 10); // En Moneda Base
                                                        
                                                                            log.audit("Governance Monitoring", "LINE 2908 - getRetencion - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                            // Nuevo - Alicuota Retencion
                                                                            retencion_iva[i].alicuota = parseFloat(objRetencionIVA.alicuotaRetencion, 10);
                                                                            ...
                                                                            if (retencion_iva[i].esRetManual) {
                                                                                ...
                                                                                log.audit("Governance Monitoring", "LINE 2969 - getRetencion - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                                var objRetencionIVA = getRetencion(entity, 'iva', retencion_iva[i].codigo, retencion_iva[i].base_calculo_retencion, id_posting_period, retencion_iva[i].imp_retenido_anterior, tasa_cambio_pago, null, considerarImportesRetAnterior, retencion_iva[i].nombreRetencion, 0, arrayDetallesRetencion);
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - objRetencionIVA: ' + JSON.stringify(objRetencionIVA) + ' ** TIEMPO' + new Date());
                                                                                retencion_iva[i].importe_retencion = objRetencionIVA.importeRetencion;
                                                                                retencion_iva[i].importe_retencion = parseFloat(retencion_iva[i].importe_retencion, 10); // En Moneda Base
                                                                                log.audit("Governance Monitoring", "LINE 2974 - getRetencion - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                                // Nuevo - Alicuota Retencion
                                                                                retencion_iva[i].alicuota = parseFloat(objRetencionIVA.alicuotaRetencion, 10);
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - retencion_iva[i].alicuota: ' + retencion_iva[i].alicuota + ' ** TIEMPO' + new Date());
                                                                                log.debug('L54 - Calculo Retenciones', 'CALCULARETENCIONES - retencion_iva[i].importe_retencion: ' + retencion_iva[i].importe_retencion + ' ** TIEMPO' + new Date());
                                                                    ...
                                                                    respuestaRetenciones.detalleAcumulados.push(extraerAcumuladoPorJurisdiccion(retencion_iva[i], entity, subsidiariaPago, id_posting_period, tasa_cambio_pago));
                                                                }
                                                                ...
                                                                log.audit("Governance Monitoring", "LINE 2891 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                        ...
                                                        if (esAgenteRetencionIIBB) {
                                                            ...
                                                            if (estaInscriptoRegimenIIBB) {
                                                                ...
                                                                let existenRetParaTodasJurisd = resultRetenJurisdMens.existenRetParaTodasJurisd;

                                                                // Funcion que permite obtener los registros de los acumulados del periodo por proveedor en el nuevo RT de acumulados por retenciones
                                                                let infoAcumPorJurisdicciones = (!isEmpty(jurisdAcumuladas) && jurisdAcumuladas.length > 0 && !existenRetParaTodasJurisd) ? obtenerAcumPorJurisdicciones(entity, id_posting_period, subsidiariaPago, jurisdAcumuladas) : null;    
                                                                
                                                                log.debug('calculoRetenciones', `infoAcumPorJurisdicciones : ${JSON.stringify(infoAcumPorJurisdicciones)}`);
                                                                ...
                                                                for (var i = 0; codigosRetencionIIBB != null && codigosRetencionIIBB.infoRet != null && i < codigosRetencionIIBB.infoRet.length; i++) {
                                                                    ...
                                                                    if (!isEmpty(codigosRetencionIIBB.infoRet[i].codigo)) {
                                                                        ...
                                                                        let existeRetJurisdiccionActual = existenRetParaTodasJurisd;
                                                                        log.debug('en el viejo oeste', JSON.stringify(codigosRetencionIIBB.infoRet[i]))


                                                                        if (!isEmpty(codigosRetencionIIBB.infoRet[i].baseCalcAcumulada) && codigosRetencionIIBB.infoRet[i].baseCalcAcumulada && !existenRetParaTodasJurisd) {
                                                                            
                                                                            existeRetJurisdiccionActual = validarRetPorJurisdiccion(resultRetenJurisdMens.registros, codigosRetencionIIBB.infoRet[i].jurisdiccion);

                                                                            // Nuevo segmento para validar si el importe acumulado de la jurisdiccion supera el importe minimo de retencion
                                                                            if (!existeRetJurisdiccionActual && !isEmpty(infoAcumPorJurisdicciones) && !infoAcumPorJurisdicciones.error) {
                                                                                    ...
                                                                                    var detalleAcumJurisdiccion = validarAcumPorJurisdiccion(infoAcumPorJurisdicciones.registros, codigosRetencionIIBB.infoRet[i].jurisdiccion);
                                                                                    ...
                                                                                    if (!isEmpty(detalleAcumJurisdiccion) && !detalleAcumJurisdiccion.error && detalleAcumJurisdiccion.registro.length > 0) {
                                                                                        ...
                                                                                        codigosRetencionIIBB.infoRet[i].importe_factura_pagar += parseFloat(detalleAcumJurisdiccion.registro[0].impBaseCalculoAcumulada, 10);
                                                                        ...
                                                                        if (!isEmpty(codigosRetencionIIBB.infoRet[i].criterioPorcentajeEspecial) && codigosRetencionIIBB.infoRet[i].criterioPorcentajeEspecial == '1') {
                                                                            ...
                                                                            codigosRetencionIIBB.infoRet[i].importe_factura_pagar = parseFloat(parseFloat(codigosRetencionIIBB.infoRet[i].importe_factura_pagar, 10) * parseFloat(porcentajeEspecialUtilizarBI, 10), 10);
                                                                        ...
                                                                        if (isEmpty(codigosRetencionIIBB.infoRet[i].impMinBaseCalculoRet) || parseFloat(codigosRetencionIIBB.infoRet[i].impMinBaseCalculoRet, 10) == 0 || 
                                                                            (!isEmpty(codigosRetencionIIBB.infoRet[i].baseCalcAcumulada) && codigosRetencionIIBB.infoRet[i].baseCalcAcumulada && existeRetJurisdiccionActual) ||
                                                                            parseFloat(codigosRetencionIIBB.infoRet[i].importe_factura_pagar, 10) > parseFloat(codigosRetencionIIBB.infoRet[i].impMinBaseCalculoRet, 10)) {
                                                                        ...
                                                                        log.audit("valoresfinales ---- porcentaje especial", "LINE 2423 - superaMinBaseCalc: " + superaMinBaseCalc + " - porcentaje = " + codigosRetencionIIBB.infoRet[i].porcentajeEspecialUtilizarBI + ' --- criterio: ' + codigosRetencionIIBB.infoRet[i].criterioPorcentajeEspecial);
                                                                        ...
                                                                        if (superaMinBaseCalc) {
                                                                                    ...
                                                                                    if (!isEmpty(codigosRetencionIIBB.infoRet[i].estadoInscripcionPadron) && !isEmpty(codigosRetencionIIBB.infoRet[i].idConvenioLocal) && codigosRetencionIIBB.infoRet[i].estadoInscripcionPadron == codigosRetencionIIBB.infoRet[i].idConvenioLocal) {
                                                                                        ...
                                                                                        if (!isEmpty(tipoContribuyenteIVA) && !isEmpty(codigosRetencionIIBB.infoRet[i].idResponsableInscripto) && tipoContribuyenteIVA == codigosRetencionIIBB.infoRet[i].idResponsableInscripto) {
                                                                                            ...
                                                                                            if (!isEmpty(tipoContribuyenteIVA) && !isEmpty(codigosRetencionIIBB.infoRet[i].idMonotrotributista) && tipoContribuyenteIVA == codigosRetencionIIBB.infoRet[i].idMonotrotributista) {
                                                                                                ...
                                                                                                log.error('DEBUG', 'No se calculará retención de TUCUMÁN; el proveedor tiene que ser Responsable Inscripto o Monotributista y la jurisdicción de entrega: TUCUMÁN, para aplicar el Padrón.');
                                                                                            }
                                                                                        }
                                                                                    } else {
                                                                            ...
                                                                            if (!isEmpty(porcentajeFinal) && !ignorarRetencionTucuman && !porcentajeAlicuotaEspecialCero && !isNaN(porcentajeFinal) && (porcentajeFinal >= 0 && codigosRetencionIIBB.infoRet[i].jurisdiccionCodigo == 924 || porcentajeFinal > 0) && 
                                                                                !isNaN(codigosRetencionIIBB.infoRet[i].importe_factura_pagar) && (((parseFloat(codigosRetencionIIBB.infoRet[i].importe_factura_pagar, 10) >= 0) && 
                                                                                (codigosRetencionIIBB.infoRet[i].jurisdiccionCodigo == 924)) || (parseFloat(codigosRetencionIIBB.infoRet[i].importe_factura_pagar, 10) > 0))) {
                                                                    ...
                                                                    respuestaRetenciones.detalleAcumulados.push(extraerAcumuladoPorJurisdiccion(codigosRetencionIIBB.infoRet[i], entity, subsidiariaPago, id_posting_period, tasa_cambio_pago));
                                                                }
                                                            }
                                                        }//FIN if (esAgenteRetencionIIBB)

                                                        log.audit("Governance Monitoring", "LINE 3168 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                        ...
                                                        log.audit("Governance Monitoring", "LINE 3172 - Remaining Usage = " + script.getRemainingUsage() + ' --- time: ' + new Date());
                                                                ...
                                                                for (var i = 0; retencion_ganancias != null && i < retencion_ganancias.length; i++) {
                                                                    ...
                                                                    if ((parseFloat(retencion_ganancias[i].importe_retencion, 10) > 0.00) && (parseFloat(retencion_ganancias[i].base_calculo_retencion, 10) > 0.00)) {
                                                                        ...
                                                                        if (informacionRetencion != null) {
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2154 - Retención Ganancias - retencion_ganancias[i].importe_retencion: ' + retencion_ganancias[i].importe_retencion + ' --- tasa_cambio_pago: ' + tasa_cambio_pago + ' --- ÍNDICE i: ' + i);
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2156 - Retención Ganancias - convertToInteger(tasa_cambio_pago): ' + convertToInteger(tasa_cambio_pago) + ', convertToInteger(retencion_ganancias[i].importe_retencion)' + convertToInteger(retencion_ganancias[i].importe_retencion));
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2160 - Retención Ganancias - imp_retencion_aux: ' + imp_retencion_aux + ', cantDecImpRetCodigoRet: ' + cantDecImpRetCodigoRet + ', cantDecTasaCambioPago: ' + cantDecTasaCambioPago + ', cantDecTotalImpRetAux: ' + cantDecTotalImpRetAux + ', cantDecImpRetAux: ' + cantDecImpRetAux + ', cantDecImpRetFinal: ' + cantDecImpRetFinal + ', imp_retencion: ' + imp_retencion);
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2177 - Retención Ganancias - imp_retencion cortado a 14 dígitos: ' + imp_retencion.toString().substr(0, 14));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2178 - Retención Ganancias - imp_retencion: ' + imp_retencion);
                                                                ...
                                                                for (var i = 0; retencion_suss != null && i < retencion_suss.length; i++) {
                                                                    ...
                                                                    if ((parseFloat(retencion_suss[i].importe_retencion, 10) > 0.00) && (parseFloat(retencion_suss[i].base_calculo_retencion, 10) > 0.00)) {
                                                                        ...
                                                                        if (informacionRetencion != null) {
                                                                            ...
                                                                            var cantDecTasaCambioPago = countDecimales(parseFloat(tasa_cambio_pago, 10));
                                                                            var cantDecTotalImpRetAux = cantDecImpRetCodigoRet - cantDecTasaCambioPago;
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2250 - Retención SUSS - retencion_suss[i].importe_retencion: ' + retencion_suss[i].importe_retencion + ' --- tasa_cambio_pago: ' + tasa_cambio_pago + ' --- ÍNDICE i: ' + i);
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2252 - Retención SUSS - convertToInteger(tasa_cambio_pago): ' + convertToInteger(tasa_cambio_pago) + ', convertToInteger(retencion_suss[i].importe_retencion)' + convertToInteger(retencion_suss[i].importe_retencion));
                                                                            var cantDecImpRetAux = countDecimales(parseFloat(imp_retencion_aux, 10));
                                                                            var cantDecImpRetFinal = cantDecImpRetAux + cantDecTotalImpRetAux;
                                                                            var imp_retencion = Math.abs(parseFloat(parseFloat(convertToInteger(imp_retencion_aux), 10) / Math.pow(10, cantDecImpRetFinal), 10));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2256 - Retención SUSS - imp_retencion_aux: ' + imp_retencion_aux + ', cantDecImpRetCodigoRet: ' + cantDecImpRetCodigoRet + ', cantDecTasaCambioPago: ' + cantDecTasaCambioPago + ', cantDecTotalImpRetAux: ' + cantDecTotalImpRetAux + ', cantDecImpRetAux: ' + cantDecImpRetAux + ', cantDecImpRetFinal: ' + cantDecImpRetFinal + ', imp_retencion: ' + imp_retencion);
                                                                            var monto_suj_ret_moneda_local = 0.0;
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2273 - Retención SUSS - imp_retencion cortado a 14 dígitos: ' + imp_retencion.toString().substr(0, 14));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2274 - Retención SUSS - imp_retencion: ' + imp_retencion);
                                                                ...
                                                                for (var i = 0; retencion_iva != null && i < retencion_iva.length; i++) {
                                                                    ...
                                                                    if ((parseFloat(retencion_iva[i].importe_retencion, 10) > 0.00) && (parseFloat(retencion_iva[i].base_calculo_retencion, 10) > 0.00)) {
                                                                        ...
                                                                        if (informacionRetencion != null) {
                                                                            ...
                                                                            var cantDecTasaCambioPago = countDecimales(parseFloat(tasa_cambio_pago, 10));
                                                                            var cantDecTotalImpRetAux = cantDecImpRetCodigoRet - cantDecTasaCambioPago;
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2348 - Retención IVA - retencion_iva[i].importe_retencion: ' + retencion_iva[i].importe_retencion + ' --- tasa_cambio_pago: ' + tasa_cambio_pago + ' --- ÍNDICE i: ' + i);
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2350 - Retención IVA - convertToInteger(tasa_cambio_pago): ' + convertToInteger(tasa_cambio_pago) + ', convertToInteger(retencion_iva[i].importe_retencion)' + convertToInteger(retencion_iva[i].importe_retencion));
                                                                            var cantDecImpRetAux = countDecimales(parseFloat(imp_retencion_aux, 10));
                                                                            var cantDecImpRetFinal = cantDecImpRetAux + cantDecTotalImpRetAux;
                                                                            var imp_retencion = Math.abs(parseFloat(parseFloat(convertToInteger(imp_retencion_aux), 10) / Math.pow(10, cantDecImpRetFinal), 10));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2354 - Retención IVA - imp_retencion_aux: ' + imp_retencion_aux + ', cantDecImpRetCodigoRet: ' + cantDecImpRetCodigoRet + ', cantDecTasaCambioPago: ' + cantDecTasaCambioPago + ', cantDecTotalImpRetAux: ' + cantDecTotalImpRetAux + ', cantDecImpRetAux: ' + cantDecImpRetAux + ', cantDecImpRetFinal: ' + cantDecImpRetFinal + ', imp_retencion: ' + imp_retencion);
                                                                            var monto_suj_ret_moneda_local = 0.0;
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2371 - Retención IVA - imp_retencion cortado a 14 dígitos: ' + imp_retencion.toString().substr(0, 14));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2372 - Retención IVA - imp_retencion: ' + imp_retencion);
                                                        ...
                                                        if (esAgenteRetencionIIBB) {
                                                            ...
                                                            if (estaInscriptoRegimenIIBB) {
                                                                ...
                                                                for (var i = 0; codigosRetencionIIBB != null && codigosRetencionIIBB.infoRet != null && i < codigosRetencionIIBB.infoRet.length; i++) {
                                                                    log.debug('RetencionesIBBB for', JSON.stringify(codigosRetencionIIBB.infoRet[i]))
                                                                    if ((parseFloat(codigosRetencionIIBB.infoRet[i].importe_retencion, 10) > 0.00) || (parseFloat(codigosRetencionIIBB.infoRet[i].importe_retencion, 10) >= 0.00 && codigosRetencionIIBB.infoRet[i].jurisdiccionCodigo == 924) && (parseFloat(codigosRetencionIIBB.infoRet[i].base_calculo, 10) >= 0.00 && codigosRetencionIIBB.infoRet[i].jurisdiccionCodigo == 924)) {
                                                                        log.debug('RetencionesIBBB if', JSON.stringify(codigosRetencionIIBB.infoRet[i]))
                                                                        var informacionRetencion = null;
                                                                        var retencionOk = false;
                                                                        ...
                                                                        if (informacionRetencion != null) {
                                                                            ...
                                                                            var cantDecTasaCambioPago = countDecimales(parseFloat(tasa_cambio_pago, 10));
                                                                            var cantDecTotalImpRetAux = cantDecImpRetCodigoRet - cantDecTasaCambioPago;
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2448 - codigosRetencionIIBB.infoRet[i].importe_retencion: ' + codigosRetencionIIBB.infoRet[i].importe_retencion + ' --- tasa_cambio_pago: ' + tasa_cambio_pago);
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2450 - convertToInteger(tasa_cambio_pago): ' + convertToInteger(tasa_cambio_pago) + ', convertToInteger(codigosRetencionIIBB.infoRet[i].importe_retencion)' + convertToInteger(codigosRetencionIIBB.infoRet[i].importe_retencion));
                                                                            var cantDecImpRetAux = countDecimales(parseFloat(imp_retencion_aux, 10));
                                                                            var cantDecImpRetFinal = cantDecImpRetAux + cantDecTotalImpRetAux;
                                                                            var imp_retencion = Math.abs(parseFloat(parseFloat(convertToInteger(imp_retencion_aux), 10) / Math.pow(10, cantDecImpRetFinal), 10));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2454 - imp_retencion_aux: ' + imp_retencion_aux + ', cantDecImpRetCodigoRet: ' + cantDecImpRetCodigoRet + ', cantDecTasaCambioPago: ' + cantDecTasaCambioPago + ', cantDecTotalImpRetAux: ' + cantDecTotalImpRetAux + ', cantDecImpRetAux: ' + cantDecImpRetAux + ', cantDecImpRetFinal: ' + cantDecImpRetFinal + ', imp_retencion: ' + imp_retencion);
                                                                            var monto_suj_ret_moneda_local = 0.0;
                                                                            ...
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2481 - imp_retencion cortado a 14 dígitos: ' + imp_retencion.toString().substr(0, 14));
                                                                            log.debug('l54 - calculo retenciones', 'LINE 2482 - imp_retencion: ' + imp_retencion);
                                                        ...
                                                        errorGeneral = true;
                                                        respuestaRetenciones.error = true;
                                                        respuestaRetenciones.mensajeError.push(codigosRetencionIIBB.mensaje);
                            ...
                            if (validacionCalculoRet == null) {
                                ...
                                respuestaRetenciones.error = true;
                                respuestaRetenciones.mensajeError.push(validacionCalculoRet.mensaje);
                            }
                        }
                    } else {
                ...
                log.error('L54 - Calculo Retenciones', 'CALCULARETENCIONES - Excepción Calcula Retenciones. Excepción: ' + e.message);
            }
        ...
        let extraerAcumuladoPorJurisdiccion = (infoCodigosRetencionIIBB, entity, subsidiariaPago, id_posting_period, tasa_cambio_pago) => {
            ...
            const proceso = 'extraerAcumuladoPorJurisdiccion';
            let infoAcumulado = {};

            log.debug(proceso, `INICIO - extraerAcumuladoPorJurisdiccion / entity: ${entity} / subsidiariaPago: ${subsidiariaPago} / id_posting_period: ${id_posting_period} / infoCodigosRetencionIIBB: ${JSON.stringify(infoCodigosRetencionIIBB)}`);

            try {
                if (!isEmpty(infoCodigosRetencionIIBB)) {
                    infoAcumulado.proveedor = entity;
                    infoAcumulado.periodo = id_posting_period;
                    infoAcumulado.subsidiaria = subsidiariaPago;
                    infoAcumulado.baseCalculo = parseFloat(parseFloat(infoCodigosRetencionIIBB.importe_factura_pagar, 10) / parseFloat(tasa_cambio_pago, 10), 10);
                    if(!isEmpty(infoCodigosRetencionIIBB.jurisdiccion)){
                        infoAcumulado.jurisdiccion = infoCodigosRetencionIIBB.jurisdiccion;
                    }
                    infoAcumulado.codigo = infoCodigosRetencionIIBB.codigo;
                    infoAcumulado.tipoCambio = tasa_cambio_pago;
                }
            } catch (error) {
                log.error(proceso, `Error NetSuite Excepcion - extraerAcumuladoPorJurisdiccion, detalles: ${error.message}`);
            }

            log.debug(proceso, `FIN - extraerAcumuladoPorJurisdiccion - infoAcumulado: ${infoAcumulado}`);
        ...
        function unificarJurisdPorLineasInscripcion(objEstadoInscripcionJurIIBB, infoLineasJurisdiccionesIIBB, importeNetoPagado, importeBrutoPagado, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia) {

            const proceso = 'unificarJurisdPorLineasInscripcion';
            let objJurisdUnificiado = {}
            ...
            try {
                if (!isEmpty(objEstadoInscripcionJurIIBB) && objEstadoInscripcionJurIIBB.jurisdicciones.length > 0) {
                        ...
                        for (let i = 0; i < objEstadoInscripcionJurIIBB.jurisdicciones.length; i++) {
                            ...
                            for (let j = 0; !isEmpty(infoLineasJurisdiccionesIIBB) && j < infoLineasJurisdiccionesIIBB.length; j++) {
                                if (objEstadoInscripcionJurIIBB.jurisdicciones[i].jurisdiccion == infoLineasJurisdiccionesIIBB[j].jurisdiccion &&  
                                    ...
                                    infoLineasJurisdiccionesIIBB[j].certExencion = objEstadoInscripcionJurIIBB.jurisdicciones[i].certExencion;
                                    ...
                                    infoLineasJurisdiccionesIIBB[j].aplicaCalcRetPorInscrip = objEstadoInscripcionJurIIBB.jurisdicciones[i].aplicaCalcRetPorInscrip;
                            ...
                            if (!encontrado && !objEstadoInscripcionJurIIBB.jurisdicciones[i].esJurisdEntrega) {
                                objJurisdUnificiado.jurisdicciones.push(unificarInfoJurisdiccion(objEstadoInscripcionJurIIBB.jurisdicciones[i], importeNetoPagado, importeBrutoPagado, paramNoAplicaProvincia));
                            }
                    ...
                    log.error(proceso, `No se continua con el calculo de retenciones porque no esta inscripto en ninguna jurisdiccion`);
                }
            } catch (error) {
        ...
        function unificarInfoJurisdiccion(infoJurisdiccion, importeNetoPagado, importeBrutoPagado, paramNoAplicaProvincia) {

            const proceso = 'unificarInfoJurisdiccion';
            log.debug(proceso, `INICIO - unificarInfoJurisdiccion`);

            try {
                infoJurisdiccion.importeNetoLinea = parseFloat(importeNetoPagado, 10);
                ...
                infoJurisdiccion.importeBrutoLinea = importeBrutoPagado;
        ...
        function validarAcumPorJurisdiccion(arrayRetAcumuladas, jurisdiccion) {

            var proceso = 'validarAcumPorJurisdiccion';
            var respuesta = { error: false, mensaje: '', registro: [] };

            log.debug(proceso, 'INICIO - validarPercJurisdiccion / arrayRetAcumuladas: ' + JSON.stringify(arrayRetAcumuladas) + ' / jurisdiccion: ' + jurisdiccion);

            try {
                if (!isEmpty(arrayRetAcumuladas) && arrayRetAcumuladas.length > 0) {
                    var result = arrayRetAcumuladas.filter(function (obj) {
                        return (obj.jurisdiccion === jurisdiccion)
                    });

                    if (!isEmpty(result) && result.length > 0) {
                        respuesta.registro.push(result[0]);
                    }
                }
            } catch (error) {
        ...
        function obtenerAcumSinJurisdicciones(entidad, periodo, subsidiaria, codigoRet) {
            ...
            let proceso = 'obtenerAcumPorJurisdicciones';
            let respuesta = { error: false, mensaje: '', registros: [] };
            log.debug(proceso, 'INICIO - obtenerAcumPorJurisdicciones / entidad: ' + entidad + ' / periodo: ' + periodo + ' / subsidiaria: ' + subsidiaria + ' / codigoRet: ' + JSON.stringify(codigoRet));
            ...
            try {
                ...
                if (!isEmpty(entidad)) {
                    let filtro = {};
                    filtro.name = 'custrecord_l54_acum_ret_proveedor';
                ...
                if (!isEmpty(subsidiaria)) {
                    let filtro1 = {};
                    filtro1.name = 'custrecord_l54_acum_ret_subsidiaria';
                ...
                if (!isEmpty(periodo)) {
                    let filtro2 = {};
                    filtro2.name = 'custrecord_l54_acum_ret_periodo';
                    filtro2.operator = 'IS';
                    filtro2.values = periodo;
                    filtros.push(filtro2);
                }

                if (!isEmpty(codigoRet)) {
                    let filtro3 = {};
                    filtro3.name = 'custrecord_l54_acum_ret_cod_ret';
                    filtro3.operator = 'ANYOF';
                    filtro3.values = codigoRet;
                    filtros.push(filtro3);
                }

                let filtro4 = {};
                filtro4.name = 'custrecord_l54_acum_ret_anulado';
                filtro4.operator = 'IS';
                filtro4.values = 'F';
                filtros.push(filtro4);
                

                let objResultSet = utilidades.searchSavedPro('customsearch_l54_script_acum_por_reten_2', filtros);

                if (objResultSet.error) {
                    log.error(proceso, 'Error en consulta de *** Script  / AR - Acumulados por Retenciones');
                ...
                if (!utilidades.isEmpty(resultSet) && resultSet.length > 0) {
                    for (let i = 0; i < resultSet.length; i++) {
                        let objInfo = {};
                        objInfo.idInterno = resultSet[i].getValue({ name: resultSearch.columns[0] });
                        objInfo.jurisdiccion = resultSet[i].getValue({ name: resultSearch.columns[1] });
                        objInfo.impBaseCalculoAcumulada = resultSet[i].getValue({ name: resultSearch.columns[4] });
                        objInfo.codigoRetención = resultSet[i].getValue({ name: resultSearch.columns[5] });
                ...
                respuesta.error = true;
                respuesta.mensaje = 'Error excepcion al obtener todos los acumulados del proveedor pagados en el periodo del pago actual - detalles: ' + error.message;
                log.error(proceso, respuesta.mensaje);
            }
        ...
        function obtenerAcumPorJurisdicciones(entidad, periodo, subsidiaria, jurisdicciones) {
            ...
            try {
                ...
                if (!isEmpty(jurisdicciones)) {
                    let filtro3 = {};
                    filtro3.name = 'custrecord_l54_acum_ret_jurisdiccion';
                    filtro3.operator = 'ANYOF';
                    filtro3.values = jurisdicciones;
                    filtros.push(filtro3);
                }
                ...
                let objResultSet = utilidades.searchSavedPro('customsearch_l54_script_acum_por_retenc', filtros);
        ...
        let validarRetPorJurisdiccion = (retencionesGeneradas, jurisdiccion) => {

            const proceso = 'validarRetPorJurisdiccion';
            let existeRet = false;

            try {
                if (!isEmpty(retencionesGeneradas) && retencionesGeneradas.length > 0) {
                    let result = retencionesGeneradas.filter(obj => {
                        return (obj.jurisdiccion == jurisdiccion)
                    })

                    existeRet = !isEmpty(result) && result.length > 0 ? true : false;
                }
            } catch (error) {
                log.error(proceso, `Error NetSuite Excepcion - validarRetPorJurisdiccion, detalles: ${error.message}`);
            }
        ...
        */
        function extraerJurisdiccionesGenerales(jurisdiccionesProveedor, jurisdiccionesCabecera) {
            ...
            try {
                ...
                log.debug('extraerJurisdiccionesGenerales', 'arrayOfJurisdiccionesProveedor: ' + JSON.stringify(arrayOfJurisdiccionesProveedor));
                log.debug('extraerJurisdiccionesGenerales', 'arrayOfJurisdiccionesCabecera: ' + JSON.stringify(arrayOfJurisdiccionesCabecera));
                ...
                log.debug('extraerJurisdiccionesGenerales', 'LINE 1331 - Jurisdicciones que no son del PROVEEDOR: ' + JSON.stringify(arrayJurisdiccionesGenerales));
        ...
        */
        function jurisdiccionesNotValid(objEstadoInscripcionJurIIBB, arrayJurisdicciones, resultsNetosJurisdiccion) {
            ...
            try {
                ...
                log.debug('jurisdiccionesNotValid', 'line 1396 - arrayOfJurisdiccionesProveedor and IIBB config gen: ' + JSON.stringify(arrayOfJurisdiccionesProveedor));
                ...
                log.debug('jurisdiccionesNotValid', 'line 1406 - arrayJurisdiccionesNotValidAux: ' + JSON.stringify(arrayJurisdiccionesNotValidAux));
                ...
                log.debug('jurisdiccionesNotValid', 'line 1416 - arrayJurisdiccionesNotValid: ' + JSON.stringify(arrayJurisdiccionesNotValid));
        ...
        function validarCalculosRetenciones(informacionPago) {
            log.debug('INICIO - validarCalculosRetenciones', 'Parametros - informacionPago: ' + JSON.stringify(informacionPago) + ' - Subsidiaria: ' + informacionPago.subsidiaria);
            ...
            log.debug('FIN - validarCalculosRetenciones', JSON.stringify(validacion));
            return validacion;
        ...
        function consultaDatosImpositivos(subsidiaria) {
            log.debug('INICIO - consultaDatosImpositivos', 'Parámetros - subsidiaria: ' + subsidiaria);
            ...
            if (!isEmpty(searchResult)) {
                ...
                log.error('FIN - consultaDatosImpositivos', 'No se encuentra registrada la Configuración de Datos Impositivos para la subsidiaria ' + subsidiariaSS + ' .');
                return false;
            }
        ...
        function obtenerJurisdiccionesAgenteRetencion(subsidiaria) {
            log.debug('INICIO - obtenerJurisdiccionesAgenteRetencion', 'Parámetros - subsidiaria: ' + subsidiaria);
            ...
            log.debug('FIN - obtenerJurisdiccionesAgenteRetencion', 'RETURN - resultadoJurisdicciones: ' + JSON.stringify(resultadoJurisdicciones));
        ...
        function getProveedorInscriptoRegimenIIBB(id_proveedor, idEstadoExento, jurisdiccionesIIBB, jurisdiccionesEntregaFacturas, idTipoContribIIBBDefault, idTipoContribIIBBDefaultText, infoJurisdiccionesConfigGeneral, trandate, paramJurisdTucuman) {

            var proceso = 'L54 - Calculo Retenciones';
            log.audit('INICIO - getProveedorInscriptoRegimenIIBB', 'Parámetros - id_proveedor: ' + id_proveedor + ', idEstadoExento: ' + idEstadoExento + ', jurisdiccionesIIBB: ' + JSON.stringify(jurisdiccionesIIBB) + ' - jurisdiccionesEntregaFacturas: ' + JSON.stringify(jurisdiccionesEntregaFacturas) + ' idTipoContribIIBBDefault: ' + idTipoContribIIBBDefault + ' - idTipoContribIIBBDefaultText: ' + idTipoContribIIBBDefaultText + ' - infoJurisdiccionesConfigGeneral: ' + JSON.stringify(infoJurisdiccionesConfigGeneral) + ' - trandate: ' + trandate);
            ...
            if (jurisdiccionesIIBB != null) {
                            ...
                            if (estado_regimen != idEstadoExento) {
                                if ((isEmpty(fecha_caducidad_parseada)) || (!isEmpty(fecha_caducidad_parseada) && (fecha_caducidad_parseada < trandate))) {
                                    ...
                                    if (jurisdiccionesAux.indexOf(jurisdiccion) == -1) {
                                        ...
                                        estadoInscripcionProveedor.jurisdicciones[indiceJurisdicciones].esJurisdEntrega = false;
                                        estadoInscripcionProveedor.jurisdicciones[indiceJurisdicciones].aplicaCalcRetPorInscrip = aplicaCalcRetPorInscrip;
                ...
                if (!isEmpty(jurisdiccionesEntregaFacturas) && jurisdiccionesEntregaFacturas.length > 0 && !jurisdiccionGeneral) {
                    for (var i = 0; i < jurisdiccionesEntregaFacturas.length; i++) {
                        ...
                        let aplicaCalcRetPorInscrip = jurisdiccionesEntregaFacturas[i].aplicaCalcRetPorInscrip;
                ...
                if (!utilidades.isEmpty(infoJurisdiccionesConfigGeneral) && infoJurisdiccionesConfigGeneral.length > 0 && !jurisdiccionGeneral) {
                    ...
                    let aplicaCalcRetPorInscrip = false;
                    let encontrado = false;

                    // Verifico si la jurisdicción es de TUCUMÁN para sacar sus datos.
                    for (var i = 0; i < infoJurisdiccionesConfigGeneral.length && !encontrado; i++) {
                        if (infoJurisdiccionesConfigGeneral[i].idJurisdiccion == paramJurisdTucuman) {
                            jurisdiccionTucuman = infoJurisdiccionesConfigGeneral[i].idJurisdiccion;
                            jurisdiccionTucumanText = infoJurisdiccionesConfigGeneral[i].jurisdiccionTexto;
                            jurisdiccionTucumanCodigo = infoJurisdiccionesConfigGeneral[i].codigoJurisdiccion;
                            aplicaCalcRetPorInscrip = infoJurisdiccionesConfigGeneral[i].aplicaCalcRetPorInscrip;
                            encontrado = true;
                            break;
                        }
                    }
                    ...
                    if (encontrado && (jurisdiccionesAux.length == 0 || jurisdiccionesAux.indexOf(jurisdiccionTucuman) == -1)) {
        ...
        function getProveedorInscriptoRegimen(id_proveedor, estadoExentoGan, estadoExentoSUSS, estadoExentoIVA, trandate) {
            log.debug('INICIO - getProveedorInscriptoRegimen', 'Parámetros - id_proveedor: ' + id_proveedor + ', estadoExentoGan: ' + estadoExentoGan + ', estadoExentoSUSS: ' + estadoExentoSUSS + ', estadoExentoIVA: ' + estadoExentoIVA + ' - trandate: ' + trandate);
        ...
        function parametrizacionRetenciones(subsidiaria) {
            log.debug('INICIO - parametrizacionRetenciones', 'Parámetros - subsidiaria: ' + subsidiaria);
            ...
            log.debug('FIN - parametrizacionRetenciones', 'RETURN - informacionCodigosVB: ' + JSON.stringify(informacionCodigosVB));
        ...
        function obtenerTotalesTransacciones(entidad, billsPagar, subsidiaria) {
            ...
            try {
                log.debug('INICIO - obtenerTotalesTransacciones', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria);
                ...
                log.debug('FIN - obtenerTotalesTransacciones', 'RETURN - informacionTotalesFacturas: ' + JSON.stringify(informacionTotalesFacturas));
            } catch (error) {
        ...
        function obtener_arreglo_codigos_ret_vendorbill(entidad, billsPagar, subsidiaria, esONG) {
            log.debug('INICIO - obtener_arreglo_codigos_ret_vendorbill', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - es ONG: ' + esONG);
            ...
            if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                ...
                log.debug('L54 - Calculo Retenciones', 'obtener_arreglo_codigos_ret_vendorbill - No se encuentro informacion para las bills recibidas por parametro');
            }

            log.debug('FIN - obtener_arreglo_codigos_ret_vendorbill', 'RETURN - informacionCodigosVB: ' + JSON.stringify(informacionCodigosVB));
            return informacionCodigosVB;
        }
        ...
        function obtener_arreglo_netos_vendorbill(entidad, billsPagar, subsidiaria, sinJurisdiccionUnica, esONG) {
            log.debug('INICIO - obtener_arreglo_netos_vendorbill', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - sinJurisdiccionUnica: ' + sinJurisdiccionUnica + ' - esONG: ' + esONG);
            ...
            if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                ...
                log.debug('L54 - Calculo Retenciones', 'obtener_arreglo_netos_vendorbill - No se encuentro informacion para las bills recibidas por parametro');
            }
            log.debug('FIN - obtener_arreglo_netos_vendorbill', 'RETURN - informacionNetosVB: ' + JSON.stringify(informacionNetosVB));
        ...
        function obtener_arreglo_imp_perc(entidad, billsPagar, subsidiaria) {
            log.debug('INICIO - obtener_arreglo_imp_perc', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria);
            ...
            if (!isEmpty(completeResultSet)) {
                ...
                log.debug('L54 - Calculo Retenciones', 'obtener_arreglo_imp_perc - No se encuentro informacion para las bills recibidas por parametro');
            }

            log.debug('FIN - obtener_arreglo_imp_perc', 'RETURN - informacionImpPercVB: ' + JSON.stringify(informacionImpPercVB));
        ...
        function obtener_arreglo_iva_vendorbill(entidad, billsPagar, subsidiaria, esONG) {
            log.debug('INICIO - obtener_arreglo_iva_vendorbill', 'Parámetros - obtener_arreglo_iva_vendorbill - Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - esONG: ' + esONG);
            ...
            if (!isEmpty(completeResultSet)) {
                ...
                log.debug('L54 - Calculo Retenciones', 'obtener_arreglo_iva_vendorbill - No se encuentro informacion para las bills recibidas por parametro');
            }

            log.debug('FIN - obtener_arreglo_iva_vendorbill', 'RETURN - informacionIVAVB: ' + JSON.stringify(informacionIVAVB));
        ...
        function calcularImpBrutosPagPasadosTotalesAcumulados(entidad, billsPagar, subsidiaria, fecha_pago) {
            ...
            try {
                log.debug('INICIO - calcularImpBrutosPagPasadosTotalesAcumulados', 'Parámetros - calcularImpBrutosPagPasadosTotalesAcumulados - Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - fecha_pago: ' + fecha_pago);
                ...
                log.debug('FIN - calcularImpBrutosPagPasadosTotalesAcumulados', 'RETURN - informacionPagoPasadosFacturaAcumulado: ' + JSON.stringify(informacionPagoPasadosFacturaAcumulado));
        ...
        function obtener_imp_factura_pasado_pagada(arregloPagosFact, idVendorBill) {
            log.debug('INICIO - obtener_imp_factura_pasado_pagada', 'Parámetros - arregloPagosFact: ' + JSON.stringify(arregloPagosFact) + ', idVendorBill: ' + idVendorBill);
            ...
            log.debug('FIN - obtener_imp_factura_pasado_pagada', 'RETURN - bruto_pagado_bill: ' + bruto_pagado_bill);
            return bruto_pagado_bill;
        }

        function obtener_neto_vendorbill(arregloNetosVendorBill, idVendorBill) {
            log.debug('INICIO - obtener_neto_vendorbill', 'Parámetros - arregloNetosVendorBill: ' + JSON.stringify(arregloNetosVendorBill) + ', idVendorBill: ' + idVendorBill);
            ...
            log.debug('FIN - obtener_neto_vendorbill', 'RETURN - neto_vendorbill: ' + neto_vendorbill);
        ...
        function obtener_importe_percepcion(arregloImpPerc, idVendorBill) {
            ...
            log.debug('FIN - obtener_importe_percepcion', 'RETURN - imp_perc: ' + imp_perc);
            return imp_perc;
        }

        function obtenerTotalesFinalesTransaccion(resultsTotalesBills, idVendorBill) {
            log.debug('INICIO - obtenerTotalesFinalesTransaccion', 'Parámetros - resultsTotalesBills: ' + JSON.stringify(resultsTotalesBills) + ', idVendorBill: ' + idVendorBill);
            ...
            log.debug('FIN - obtenerTotalesFinalesTransaccion', 'RETURN - resultFactura: ' + JSON.stringify(resultFactura));
            return resultFactura;
        }

        function obtener_codigos_vendorbill(arregloCodigosVendorBill, idVendorBill, lineNumber, billsPagadasInfoLineas) {
            log.debug('INICIO - obtener_codigos_vendorbill', 'Parámetros - arregloCodigosVendorBill: ' + JSON.stringify(arregloCodigosVendorBill) + ', idVendorBill: ' + idVendorBill + ' - billsPagadasInfoLineas: ' + JSON.stringify(billsPagadasInfoLineas) + ' - lineNumber: ' + lineNumber);
            ...
            if (arregloCodigosVendorBill != null && arregloCodigosVendorBill.length > 0 && !isEmpty(idVendorBill)) {
                ...
                var resultadoCodigoRetencionLinea = billsPagadasInfoLineas.filter(function (obj) {
                    return (obj.idVendorBill == idVendorBill && obj.esRetencionPorLinea && obj.lineNumber == lineNumber);
                });

                // Se comenta esto porque las retenciones son todas por lineas, no debe convivir calculo de cabecera con el de lineas
                if (!isEmpty(resultadoCodigosRetencion) && resultadoCodigosRetencion.length > 0) {
                    ...
                    resultadoCodigos.esRetencionPorLinea = false;
                    resultadoCodigos.importeBrutoLinea = '';
                    resultadoCodigos.jurisdUtilizacion = '';
                    resultadoCodigos.nombreJurisdUtilizacion = '';
                    resultadoCodigos.jurisdOrigen = '';
                    resultadoCodigos.nombreJurisdOrigen = '';
                    resultadoCodigos.jurisdiccionEntregaID = '';
                    resultadoCodigos.jurisdiccionEntregaNombre = '';
                    resultadoCodigos.jurisdFacturacion = '';
                    resultadoCodigos.nombreJurisdFacturacion = '';
                }

                if (!utilidades.isEmpty(resultadoCodigoRetencionLinea) && resultadoCodigoRetencionLinea.length > 0) {
                    resultadoCodigos.importeTotal = resultadoCodigos.importeTotal;
                    resultadoCodigos.codigoRetGanancias = resultadoCodigoRetencionLinea[0].codRetencionGananciasLinea;
                    resultadoCodigos.codigoRetIVA = resultadoCodigos.codigoRetIVA;
                    resultadoCodigos.codigoRetSUSS = resultadoCodigoRetencionLinea[0].codRetencionSussLinea;
                    resultadoCodigos.esFacturaM = resultadoCodigos.esFacturaM;
                    resultadoCodigos.calcularSobreIVA = resultadoCodigos.calcularSobreIVA;
                    resultadoCodigos.nombreRetGanancias = resultadoCodigoRetencionLinea[0].nombreRetencionGanancias;
                    resultadoCodigos.nombreRetSUSS = resultadoCodigoRetencionLinea[0].nombreRetencionSuss;
                    resultadoCodigos.nombreRetIVA = resultadoCodigos.nombreRetIVA;
                    resultadoCodigos.esFacturaAliados = resultadoCodigos.esFacturaAliados;
                    resultadoCodigos.esRetencionPorLinea = true;
                    resultadoCodigos.importeBrutoLinea = resultadoCodigoRetencionLinea[0].importeBrutoLinea;
                    resultadoCodigos.jurisdUtilizacion = resultadoCodigoRetencionLinea[0].jurisdUtilizacion;
                    resultadoCodigos.nombreJurisdUtilizacion = resultadoCodigoRetencionLinea[0].nombreJurisdUtilizacion;
                    resultadoCodigos.jurisdOrigen = resultadoCodigoRetencionLinea[0].jurisdOrigen;
                    resultadoCodigos.nombreJurisdOrigen = resultadoCodigoRetencionLinea[0].nombreJurisdOrigen;
                    resultadoCodigos.jurisdiccionEntregaID = resultadoCodigoRetencionLinea[0].jurisdiccionEntregaID;
            ...
            log.debug('FIN - obtener_codigos_vendorbill', 'RETURN - resultadoCodigos: ' + JSON.stringify(resultadoCodigos));
            return resultadoCodigos;
        }

        function obtener_iva_vendorbill(arregloIVAVendorBill, idVendorBill) {

            log.debug('L54 - Calculo Retenciones', 'INICIO - obtener_iva_vendorbill');
            ...
            log.debug('L54 - Calculo Retenciones', 'FIN - obtener_iva_vendorbill');
            return resultadoIVA;
        }

        /* Funcion encargada de agregar informacion de las jurisdicciones por territorialidad */
        function agregarJurisdiccionesIIBB(objRetencion, jurisdiccion, importeNetoLinea, jurisdiccionTexto, esJurisdUtilizacion, esJurisdOrigen, esJurisdEntrega, importeBrutoLinea, lineNumber, esJurisdFact, infoLineaActual, paramAplicaEnLaProvincia, paramAplicaFueraProvincia, paramNoAplicaProvincia, id_vendorbill) {

            log.debug('L54 - Calculo Retenciones', 'INICIO - agregarJurisdiccionesIIBB');
            log.debug('L54 - Calculo Retenciones', 'Parámetros - objRetencion: ' + JSON.stringify(objRetencion) + ', jurisdiccion: ' + jurisdiccion + ', importeNetoLinea: ' + importeNetoLinea +
                ...
                ', importeBrutoLinea: ' + importeBrutoLinea + ', esJurisdFact: ' + esJurisdFact);

            let codigoEncontrado = false;
            let infoTerritorialidadLinea = {};
            ...
            if (!isEmpty(infoLineaActual) && !isEmpty(infoLineaActual.jurisdiccionEntregaID) && (jurisdiccion == infoLineaActual.jurisdiccionEntregaID)) {
                infoTerritorialidadLinea.critJurisdEntrega = paramAplicaEnLaProvincia;
            } else if (!isEmpty(infoLineaActual) && !isEmpty(infoLineaActual.jurisdiccionEntregaID) && (jurisdiccion != infoLineaActual.jurisdiccionEntregaID)) {
                infoTerritorialidadLinea.critJurisdEntrega = paramAplicaFueraProvincia;
            } else if (!isEmpty(infoLineaActual) && isEmpty(infoLineaActual.jurisdiccionEntregaID)) {
                infoTerritorialidadLinea.critJurisdEntrega = paramNoAplicaProvincia;
            }

            // Segmento encargado de validar si la jurisdiccion de facturacion/empresa aplica en la provincia, fuera de la provincia o no aplica en la provincia.
                ...
                if (objRetencion.length > 0) {
                    ...
                    for (var i = 0; i < objRetencion.length && codigoEncontrado == false; i++) {
                        if (objRetencion[i].jurisdiccion == jurisdiccion && objRetencion[i].critJurisdUtilizacion == infoTerritorialidadLinea.critJurisdUtilizacion &&
                            ...
                            objRetencion[i].esJurisdUtilizacion = (esJurisdUtilizacion) ? esJurisdUtilizacion : objRetencion[i].esJurisdUtilizacion;
                            objRetencion[i].esJurisdOrigen = (esJurisdOrigen) ? esJurisdOrigen : objRetencion[i].esJurisdOrigen;
                            objRetencion[i].esJurisdEntrega = (esJurisdEntrega) ? esJurisdEntrega : objRetencion[i].esJurisdEntrega;
                            objRetencion[i].esJurisdFact = (esJurisdFact) ? esJurisdFact : objRetencion[i].esJurisdFact;

                            // Valido si se ecuentra la factura en las facturas procesadas
                            var index = objRetencion[i].arrayIdsFacturas.indexOf(id_vendorbill);

                            // log.emergency(proceso, `objRetencion[i]: ${JSON.stringify(objRetencion[i])}`);

                            if (objRetencion[i].lineNumber != lineNumber || (objRetencion[i].lineNumber == lineNumber  && index == -1)) {
                                objRetencion[i].lineNumber = lineNumber;
                                objRetencion[i].arrayIdsFacturas.push(id_vendorbill);
                                objRetencion[i].importeNetoLinea += parseFloat(importeNetoLinea, 10);
                                objRetencion[i].importeBrutoLinea += parseFloat(importeBrutoLinea, 10);
                                objRetencion[i].importeNetoLinea = (parseFloat(countDecimales(objRetencion[i].importeNetoLinea), 10) > 13) ? parseFloat(parseFloat(objRetencion[i].importeNetoLinea, 10).toFixedOK(2), 10) : objRetencion[i].importeNetoLinea;
                                objRetencion[i].importeBrutoLinea = (parseFloat(countDecimales(objRetencion[i].importeBrutoLinea), 10) > 13) ? parseFloat(parseFloat(objRetencion[i].importeBrutoLinea, 10).toFixedOK(2), 10) : objRetencion[i].importeBrutoLinea;
                            }
                        }
                    }
                }
            ...
            let cantidadElementos = !isEmpty(objRetencion) && objRetencion.length > 0 ? objRetencion.length : 0;
            ...
            if (!codigoEncontrado) {
                ...
                objRetencion[cantidadElementos].arrayIdsFacturas = [];
                objRetencion[cantidadElementos].arrayIdsFacturas.push(id_vendorbill);
                objRetencion[cantidadElementos].jurisdiccion = jurisdiccion;
                objRetencion[cantidadElementos].importeNetoLinea = parseFloat(importeNetoLinea, 10);
                objRetencion[cantidadElementos].lineNumber = lineNumber;
                // objRetencion[cantidadElementos].imp_neto_factura = parseFloat(impNetoLinea, 10);
                objRetencion[cantidadElementos].jurisdiccionTexto = jurisdiccionTexto;
                objRetencion[cantidadElementos].esJurisdUtilizacion = esJurisdUtilizacion;
                objRetencion[cantidadElementos].esJurisdOrigen = esJurisdOrigen;
                objRetencion[cantidadElementos].esJurisdEntrega = esJurisdEntrega;
                objRetencion[cantidadElementos].esJurisdFact = esJurisdFact;
                objRetencion[cantidadElementos].importeBrutoLinea = parseFloat(importeBrutoLinea, 10);
                objRetencion[cantidadElementos].critJurisdUtilizacion = infoTerritorialidadLinea.critJurisdUtilizacion;
            ...
            log.debug('L54 - Calculo Retenciones', 'FIN - agregarJurisdiccionesIIBB');
        ...
        function agregarCodigoRetencion(objRetencion, codigo_retencion, importe_a_pagar, esFacturaM, impNetoFactura, calcularSobreIVA, nombreRetencion, esRetManual) {
            log.debug('INICIO - agregarCodigoRetencion', 'Parámetros - objRetencion: ' + JSON.stringify(objRetencion) + ', codigo_retencion: ' + codigo_retencion + ', importe_a_pagar: ' + importe_a_pagar + ', esFacturaM: ' + esFacturaM + ', impNetoFactura: ' + impNetoFactura + ' - nombreRetencion: ' + nombreRetencion + ' - esRetManual: ' + esRetManual);
            ...
            log.debug('FIN - agregarCodigoRetencion', 'RETURN - objRetencion: ' + JSON.stringify(objRetencion));
            return objRetencion;
        }
        ...
        function obtenerCodigosRetencionIIBB(id_proveedor, subsidiaria, objEstadosIIBB, idConfGeneral, importeNetoTotalFacturas, resultsNetosJurisdiccion, arrayJurisdiccionesEntregaUnificado, existeFacturaSinJurisdiccionEntrega, tipoContribuyenteIVA, importeBrutoFacturasProveedorNormal, jurisdiccionCordoba, importeBrutoFacturasAliados, importeBrutoTotalFacturas, paramNoAplicaProvincia,id_posting_period, ignorarTipoContribuyenteIVA) {
            ...
            try {
                log.debug('obtenerCodigosRetencionIIBB', 'INICIO - obtenerCodigosRetencionIIBB');
                log.debug('obtenerCodigosRetencionIIBB', 'Parámetros - id_proveedor: ' + id_proveedor + ' - subsidiaria: ' + subsidiaria + ' - objEstadosIIBB: ' + JSON.stringify(objEstadosIIBB) + ' - idConfGeneral: ' + idConfGeneral + ' - importeNetoTotalFacturas: ' + importeNetoTotalFacturas +
                     ' - resultsNetosJurisdiccion: ' + JSON.stringify(resultsNetosJurisdiccion) + '- tipoContribuyenteIVA: ' + tipoContribuyenteIVA + ' - importeBrutoFacturasProveedorNormal: ' + importeBrutoFacturasProveedorNormal + 
                ...
                let jurisdNoAplicaCalRetPorInscripcion = '';
                let errorAplicaCalRetPorInscrip = false;
                ...
                if (!isEmpty(objEstadosIIBB) && !isEmpty(objEstadosIIBB.jurisdicciones) && (objEstadosIIBB.jurisdicciones.length > 0) && !isEmpty(idConfGeneral)) {
                    ...
                    if (!errorObtCodigoRetIIBB && !errorParcial) {
                        ...
                        log.debug('jurisdiccionesEntrega',jurisdiccionesEntrega)
                        ...
                        for (var i = 0; i < objEstadosIIBB.jurisdicciones.length; i++) {
                            ...
                            log.debug(proceso, `LINE 7480 - jurisdiccion: ${objEstadosIIBB.jurisdicciones[i].jurisdiccion} / objEstadosIIBB.jurisdicciones: ${JSON.stringify(objEstadosIIBB.jurisdicciones)}`);
                            var codigoRetIIBB = {};
                            ...
                            if (objEstadosIIBB.jurisdicciones[i].critJurisdUtilizacion == paramNoAplicaProvincia && objEstadosIIBB.jurisdicciones[i].critJurisdEntrega == paramNoAplicaProvincia && 
                                ...
                                if (objEstadosIIBB.jurisdicciones[i].aplicaCalcRetPorInscrip) {
                                    ...
                                    errorCalcRetInscrip = true;
                                    log.error(proceso, `LINE 7546 - No se procederá a buscar la alícuota para la jurisdiccion: ${objEstadosIIBB.jurisdicciones[i].jurisdiccionTexto} porque es una jurisdicción que no aplica a cálculo de retenciones por inscripción y no está en las líneas de las facturas/notas de créditos.`);
                                }
                            } else {
                                buscarConfig = true;
                            }
                            ...
                            log.debug(proceso, `LINE 7498 - infoConfigDetalle: ${JSON.stringify(infoConfigDetalle)}`);
                            ...
                            if (!isEmpty(infoConfigDetalle) && infoConfigDetalle.length > 0) {
                                ...
                                let idRegConfigDetalle = infoConfigDetalle[0].idInterno;
                                ...
                                if (errorParcial || errorPadronExcluyente) {
                                    ...
                                    infoRet = new Object();
                                    infoRet.idRegConfigDetalle = idRegConfigDetalle;
                                    ...
                                    if (!isEmpty(calcularSobreBruto) && calcularSobreBruto) {
                                        infoRet.importe_factura_pagar = parseFloat(objEstadosIIBB.jurisdicciones[i].importeBrutoLinea, 10);
                                    } else {
                                        infoRet.importe_factura_pagar = parseFloat(objEstadosIIBB.jurisdicciones[i].importeNetoLinea, 10);
                                    }

                                    infoRet.importe_factura_pagar = (parseFloat(countDecimales(parseFloat(infoRet.importe_factura_pagar, 10)), 10) > 13) ? parseFloat(parseFloat(infoRet.importe_factura_pagar, 10).toFixedOK(2), 10) : infoRet.importe_factura_pagar;
                                    ...
                                    log.debug('obtenerCodigosRetencionIIBB', 'LINE 5396 - infoRet.importe_factura_pagar: ' + infoRet.importe_factura_pagar);
                                    ...
                                    //-------------- INICIO - VERIFICACION DE SI EXISTE UN CODIGO DE RETENCION QUE YA COINCIDA CON EL QUE SE INTENTA PROCESA -----------//////

                                    let regEncontrado = false;

                                    if (codigosRetencionIIBB.infoRet.length > 0) {
                                        for (let j = 0; j < codigosRetencionIIBB.infoRet.length && !regEncontrado; j++) {
                                            if (codigosRetencionIIBB.infoRet[j].jurisdiccion == infoRet.jurisdiccion && codigosRetencionIIBB.infoRet[j].codigo == infoRet.codigo && 
                                                codigosRetencionIIBB.infoRet[j].idRegConfigDetalle == infoRet.idRegConfigDetalle && codigosRetencionIIBB.infoRet[j].porcentajeRetencion == infoRet.porcentajeRetencion) 
                                            {
                                                regEncontrado = true;
                                                codigosRetencionIIBB.infoRet[j].importe_factura_pagar += parseFloat(infoRet.importe_factura_pagar, 10);
                                            }
                                        }
                                    }
                                    ...
                                    //-------------- FIN - VERIFICACION DE SI EXISTE UN CODIGO DE RETENCION QUE YA COINCIDA CON EL QUE SE INTENTA PROCESA -----------//////
                                ...
                                if (errorCalcRetInscrip) {
                                    
                                    errorAplicaCalRetPorInscrip = true;
                                    jurisdNoAplicaCalRetPorInscripcion += (isEmpty(jurisdNoAplicaCalRetPorInscripcion)) ? objEstadosIIBB.jurisdicciones[i].jurisdiccionTexto : ', ' + objEstadosIIBB.jurisdicciones[i].jurisdiccionTexto;
                                } else {
                                    errorObtCodigoRetIIBB = true;
                ...
                log.debug('obtenerCodigosRetencionIIBB', 'errorObtCodigoRetIIBB: ' + errorObtCodigoRetIIBB + ' - errorPadronExcluyenteGeneral: ' + errorPadronExcluyenteGeneral + ' - errorAplicaCalRetPorInscrip: ' + errorAplicaCalRetPorInscrip);
                
                if(errorObtCodigoRetIIBB){ 
                    log.debug('jurisdiccionesSinConfiguracion', jurisdiccionesSinConfiguracion)
                    var arrAux = jurisdiccionesSinConfiguracion.split(',');
                    log.debug('arrAux', arrAux)
                    var arrCorrect = [];

                    for (var i = 0; i < arrAux.length; i++) {
                        var element = arrAux[i];
                        var arrResult =codigosRetencionIIBB.infoRet.filter(function (obj) {
                            log.debug('ultimo', obj.jurisdiccionTexto )
                            return (obj.jurisdiccionTexto == element); 
                        }) 
                        log.debug('arrResult', arrResult)
                        if(arrResult.length == 0){ 
                            arrCorrect = arrCorrect.concat(element);
                        }
                    }
                    
                    if(arrAux != arrCorrect){
                        log.debug('no debio entrar', arrAux != arrCorrect)
                        if(arrCorrect.length == 0){
                            errorObtCodigoRetIIBB = false
                        }else{
                            for (var j = 0; j < arrCorrect.length; j++) {
                                log.debug('mirame')
                                if(j == 0) jurisdiccionesSinConfiguracion = arrCorrect[j];
                                else jurisdiccionesSinConfiguracion += ', ' + arrCorrect[j];
                            }
                        }
                    }
                }

                if(errorAplicaCalRetPorInscrip){
                    var arrAux = jurisdNoAplicaCalRetPorInscripcion.split(',');
                    var arrCorrect = [];

                    for (let i = 0; i < arrAux.length; i++) {
                        let element = arrAux[i];
                        let arrResult =codigosRetencionIIBB.infoRet.filter(function (obj) {
                            return (obj.jurisdiccionTexto == element); 
                        }) 
                    ...
                    if(arrCorrect.length == 0){
                        errorAplicaCalRetPorInscrip = false
                    }else{
                        for (let j = 0; j < arrCorrect.length; j++) {
                            if(j == 0) jurisdNoAplicaCalRetPorInscripcion = arrCorrect[j];
                            else jurisdNoAplicaCalRetPorInscripcion += ', ' + arrCorrect[j];
                        }
                    }
                }

                if(errorPadronExcluyenteGeneral){
                    var arrAux = jurisdiccionesExcluyentesText.split(',');
                    var arrCorrect = [];
                    ...
                    if(arrCorrect.length == 0){
                        errorPadronExcluyenteGeneral = false
                    }else{
                        for (let j = 0; j < arrCorrect.length; j++) {
                            if(j == 0) jurisdiccionesExcluyentesText = arrCorrect[j];
                            else jurisdiccionesExcluyentesText += ', ' + arrCorrect[j];
                        }
                    }
                }
                ...
                if (errorObtCodigoRetIIBB || errorPadronExcluyenteGeneral || errorAplicaCalRetPorInscrip) {
                    ...
                    if (errorAplicaCalRetPorInscrip)
                        codigosRetencionIIBB.mensaje += `No se aplica el cálculo de retenciones para las provincias: ${jurisdNoAplicaCalRetPorInscripcion}, porque es una jurisdicción en la cuál está inscripto el proveedor pero que no aplica a cálculo de retenciones por inscripción y no está en las líneas de las facturas/notas de créditos. \n`;
                }
            } catch (e) {
            ...
            log.debug('obtenerCodigosRetencionIIBB', 'FIN - obtenerCodigosRetencionIIBB');
        ...
        function getCondicion(id_proveedor) {
            ...
            log.debug('L54 - Calculo Retenciones', 'INICIO - getCondicion');
            ...
            if (!isEmpty(completeResultSet)) {
                ...
                log.debug('L54 - Calculo Retenciones', 'getCondicion - No se encuentro informacion para el proveedor recibido por parametro');
            }

            log.debug('L54 - Calculo Retenciones', 'RETURN - codRetProveedor: ' + JSON.stringify(codRetProveedor));
            log.debug('L54 - Calculo Retenciones', 'FIN - getCondicion');
        ...
        function getRetencion(id_proveedor, id_tipo_ret, codigo_retencion, base_calculo, id_posting_period, importe_retenido_ant, tasa_cambio, porcentajeRetIIBB, consImpRetAnt, nombreRetencionCalculada, baseCalculoRetencionesAcumuladas, arrayDetallesRetencion, baseCalcAcumulada, existeRetJurisdiccionActual) {

            log.debug('L54 - Calculo Retenciones', 'INICIO - getRetencion');
            ...
            if (id_tipo_ret == "iibb") {
                // valores = obtenerParametrosMatriz(arrayDetallesRetencion, base_calculo_aux, true, codigo_retencion);
                valores = obtenerParametrosMatriz(arrayDetallesRetencion, base_calculo, true, codigo_retencion);
            } else {
                // valores = obtenerParametrosMatriz(arrayDetallesRetencion, base_calculo_aux, false, codigo_retencion);
                valores = obtenerParametrosMatriz(arrayDetallesRetencion, base_calculo, false, codigo_retencion);
            }
            ...
            if (errorDetalle == false && errorAlicuotaRetencion == false && !isEmpty(porcentajeRetencion) && !isNaN(porcentajeRetencion)) {
                log.debug('getRetencion', 'base_calculo: ' + base_calculo + ', valores[excedente]: ' + valores['excedente'] + ', porcentajeRetencion: ' + porcentajeRetencion);
                ...
                log.debug('getRetencion', 'cantDecTotal: ' + cantDecTotal + ', cantDecRetencion: ' + cantDecRetencion + ', cantDecPorcentaje: ' + cantDecPorcentaje + ', convertToInteger(retencion): ' + convertToInteger(retencion) + ', convertToInteger(porcentajeRetencion): ' + convertToInteger(porcentajeRetencion) + ', convertToInteger(porcentajeRetencion): ' + convertToInteger(porcentajeRetencion) + ', retencion: ' + retencion);
                ...
                if ((retencion > parseFloat(valores['minimo_retencion'], 10)) || (id_tipo_ret == "iibb" && retencion < parseFloat(valores['minimo_retencion'], 10) && 
                    !isEmpty(baseCalcAcumulada) && baseCalcAcumulada && existeRetJurisdiccionActual)) {
            ...
            log.debug('L54 - Calculo Retenciones', 'RETURN - objRetencion: ' + JSON.stringify(objRetencion));
            log.debug('L54 - Calculo Retenciones', 'FIN - getRetencion');
            return objRetencion;
        }
        ...
        function obtenerParametrosMatriz(arrayDetallesRetencion, valor, esRetIIBB, idRetencion) {

            try {
                log.debug('obtenerParametrosMatriz', 'INICIO - obtenerParametrosMatriz');
                log.debug('obtenerParametrosMatriz', 'Parámetros - arrayDetallesRetencion: ' + JSON.stringify(arrayDetallesRetencion) + ' - valor: ' + valor + ' - esRetIIBB: ' + esRetIIBB + ' - idRetencion: ' + idRetencion);

                var resultado = new Array();
                resultado['detalle_encontrado'] = false;
                resultado['detalle_encontrado_importes'] = false;

                if (!isEmpty(arrayDetallesRetencion) && arrayDetallesRetencion.length > 0 && !isEmpty(idRetencion)) {

                    //FILTRO CODIGO RETENCION PADRE
                    idRetencion = parseInt(idRetencion);
                    if (esRetIIBB) {
                        var completeResultSet = arrayDetallesRetencion.filter(function (obj) {
                            return (obj.codigoRetencionPadre == idRetencion);
                        });
                    } else {
                        var completeResultSet = arrayDetallesRetencion.filter(function (obj) {
                            return (obj.idCodigoRetencionPadre == idRetencion);
                        });
                    }
                    ...
                    if ((!isEmpty(completeResultSet)) && (completeResultSet.length > 0)) {
                        for (var i = 0; i < completeResultSet.length; i++) {
                            ...
                            objInfoDetaParametrizacion.monto_fijo_a_retener = completeResultSet[i].monto_fijo_a_retener;//IMPORTE RETENIDO

                            objInfoDetaParametrizacion.porcentaje = completeResultSet[i].porcentaje;//PORC. ADICIONAL

                            objInfoDetaParametrizacion.excedente = completeResultSet[i].excedente;//EXCEDENTE

                            objInfoDetaParametrizacion.minimo_retencion = completeResultSet[i].minimo_retencion;//IMPORTE MINIMO RETENCION

                            objInfoDetaParametrizacion.nombreRet = completeResultSet[i].nombreRet;//NOMBRE RETENCION

                            objInfoDetaParametrizacion.importeDesde = completeResultSet[i].importeDesde;//IMPORTE DESDE

                            objInfoDetaParametrizacion.importeHasta = completeResultSet[i].importeHasta;//IMPORTE HASTA

                            log.debug('obtenerParametrosMatriz', 'obtenerParametrosMatriz - objInfoDetaParametrizacion: ' + JSON.stringify(objInfoDetaParametrizacion));
                        ...
                        resultado['detalle_encontrado'] = false;
                        log.debug('obtenerParametrosMatriz', 'obtenerParametrosMatriz - No se encontró información de detalle de parametrización para el código de retención');
                    }

                    if (!resultado['detalle_encontrado_importes'])
                        log.debug('obtenerParametrosMatriz', 'obtenerParametrosMatriz - No se encontró información para el código de retención y valor indicado');
                ...
                log.debug('obtenerParametrosMatriz', 'RETURN - resultado: ' + JSON.stringify(resultado));
                log.debug('obtenerParametrosMatriz', 'FIN - obtenerParametrosMatriz');
                return resultado;

            } catch (e) {
                log.error('obtenerParametrosMatriz', 'obtenerParametrosMatriz - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }
        ...
        function letras(c, d, u) {
            ...
            log.debug('L54 - Calculo Retenciones', 'INICIO - letras');
            ...
            if (decenas == 1) {
                ...
                log.debug('L54 - Calculo Retenciones', 'FIN - letras');
                return lc + ld;
            }
            if (decenas == 0 || decom == 0) {
        ...
        function getNumberLiteral(n) {
            ...
            log.debug('L54 - Calculo Retenciones', 'INICIO - getNumberLiteral');
        ...
        function obtener_arreglo_netos_vendorbill_jurisdiccion(entidad, billsPagar, subsidiaria, esONG) {
            log.debug('INICIO - obtener_arreglo_netos_vendorbill_jurisdiccion', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - esONG: ' + esONG);
            ...
            log.debug('FIN - obtener_arreglo_netos_vendorbill_jurisdiccion', 'RETURN - informacionNetosJurisdiccion: ' + JSON.stringify(informacionNetosJurisdiccion));
            ...
            try {
                ...
                if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                    for (var j = 0; j < completeResultSet.length; j++) {
                        ...
                        objInfoConfigDetalle.idInterno = completeResultSet[j].getValue({
                            name: resultSearch.columns[0]
                        });//ID INTERNO
                        ...
                        objInfoConfigDetalle.critJurisdUtilizacion = completeResultSet[j].getValue({
                            name: resultSearch.columns[24]
                        });
                        
                        objInfoConfigDetalle.critJurisdEntrega = completeResultSet[j].getValue({
                            name: resultSearch.columns[25]
                        });

                        objInfoConfigDetalle.critJurisdOrigen = completeResultSet[j].getValue({
        ...
        function obtener_jurisdicciones_entrega_facturas(resultsNetosVBLineas) {
            ...
            try {
                ...
                log.debug('INICIO - obtener_jurisdicciones_entrega_facturas', 'Parámetros: resultsNetosVBLineas: ' + JSON.stringify(resultsNetosVBLineas));

                for (var i = 0; i < resultsNetosVBLineas.length; i++) {
                    if (!isEmpty(resultsNetosVBLineas[i].jurisdiccionEntregaID) && !isEmpty(resultsNetosVBLineas[i].jurisdiccionEntregaCodigo)) {
                        arrayJurisdiccionesEntregaFacturas.push(resultsNetosVBLineas[i]);
                    }
                }

                log.debug('FIN - obtener_jurisdicciones_entrega_facturas', 'CALCULARETENCIONES  - jurisdiccionesEntregaFacturas: ' + JSON.stringify(arrayJurisdiccionesEntregaFacturas));
        ...
        function obtener_info_jurisdicciones_config_general(jurisdicciones) {
            ...
            try {
                ...
                log.debug('INICIO - obtener_info_jurisdicciones_config_general', 'Parámetros: jurisdicciones: ' + JSON.stringify(jurisdicciones));
                var arrayJurisdiccionesGenerales = [];
                ...
                if (objResultSet.error) {
                    log.error('obtener_info_jurisdicciones_config_general', 'Error en consulta de SS de customsearch_l54_zona_impuestos_retencio - Error: ' + objResultSet.descripcion);
                    return null;
                }
                ...
                if (!utilidades.isEmpty(resultSet) && resultSet.length > 0) {
                    ...
                    for (var i = 0; !utilidades.isEmpty(resultSet) && i < resultSet.length; i++) {
                        ...
                        info.jurisdiccionTexto = resultSet[i].getValue({ name: resultSearch.columns[1] });
                        info.codigoJurisdiccion = resultSet[i].getValue({ name: resultSearch.columns[2] });
                        info.aplicaCalcRetPorInscrip = resultSet[i].getValue({ name: resultSearch.columns[3] });

                        arrayJurisdiccionesGenerales.push(info);
                    }
                } else {
                    log.debug('obtener_info_jurisdicciones_config_general', 'obtener_info_jurisdicciones_config_general - No se obtuvieron resultados de jurisdicciones generales');
                }

                log.debug('FIN - obtener_info_jurisdicciones_config_general', 'RETURN - arrayJurisdiccionesGenerales: ' + JSON.stringify(arrayJurisdiccionesGenerales));
                return arrayJurisdiccionesGenerales;
        ...
        function obtener_arreglo_codigos_ret_vendorbill_lineas(entidad, billsPagar, subsidiaria, esONG) {

            try {
                log.debug('INICIO - obtener_arreglo_codigos_ret_vendorbill_lineas', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - es ONG: ' + esONG);

                var informacionCodigosVBLineas = [];

                //SAVE SEARCH A EJECUTAR
                var searchCodRet = search.load({
                    id: 'customsearch_l54_bill_cod_ret_lineas'
                });
                ...
                if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                    for (var i = 0; i < completeResultSet.length; i++) {
                        informacionCodigosVBLineas[i] = new Object();

                        informacionCodigosVBLineas[i].idInterno = completeResultSet[i].getValue({
                            name: resultSearch.columns[1]
                        });//ID INTERNO

                        informacionCodigosVBLineas[i].importeTotal = parseFloat(completeResultSet[i].getValue({
                            name: resultSearch.columns[2]
                        }), 10).toFixedOK(2);//IMPORTE BRUTO

                        informacionCodigosVBLineas[i].codigoRetGanancias = completeResultSet[i].getValue({
                            name: resultSearch.columns[3]
                        });//CODIGO RETENCION GANANCIAS

                        informacionCodigosVBLineas[i].codigoRetIVA = completeResultSet[i].getValue({
                            name: resultSearch.columns[4]
                        });//CODIGO RETENCION IVA

                        informacionCodigosVBLineas[i].codigoRetSUSS = completeResultSet[i].getValue({
                            name: resultSearch.columns[5]
                        });//CODIGO RETENCION SUSS

                        informacionCodigosVBLineas[i].esFacturaM = completeResultSet[i].getValue({
                            name: resultSearch.columns[6]
                        });//INDICA SI ES UNA TRANSACCION M

                        informacionCodigosVBLineas[i].calcularSobreIVA = completeResultSet[i].getValue({
                            name: resultSearch.columns[8]
                        });//INDICA SI LA BASE DE CÁLCULO SE CALCULA SOBRE IVA

                        informacionCodigosVBLineas[i].nombreRetGanancias = completeResultSet[i].getValue({
                            name: resultSearch.columns[9]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE GANANCIAS

                        informacionCodigosVBLineas[i].nombreRetSUSS = completeResultSet[i].getValue({
                            name: resultSearch.columns[10]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE SUSS

                        informacionCodigosVBLineas[i].nombreRetIVA = completeResultSet[i].getValue({
                            name: resultSearch.columns[11]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE IVA

                        informacionCodigosVBLineas[i].codRetencionGanancias = completeResultSet[i].getValue({
                            name: resultSearch.columns[12]
                        });//INDICA EL CÓDIGO DE LA RETENCIÓN DE GANANCIAS A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].nombreRetencionGanancias = completeResultSet[i].getText({
                            name: resultSearch.columns[12]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE GANANCIAS A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].codRetencionSuss = completeResultSet[i].getValue({
                            name: resultSearch.columns[13]
                        });//INDICA EL CÓDIGO DE LA RETENCIÓN DE SUSS A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].nombreRetencionSuss = completeResultSet[i].getText({
                            name: resultSearch.columns[13]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE SUSS A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].lineNumber = completeResultSet[i].getValue({
                            name: resultSearch.columns[17]
                        });//INDICA EL NRO DE LÍNEA

                        informacionCodigosVBLineas[i].importeNeto = parseFloat(completeResultSet[i].getValue({
                            name: resultSearch.columns[18]
                        }), 10).toFixedOK(2);//INDICA EL IMPORTE NETO DE LA LÍNEA

                        informacionCodigosVBLineas[i].jurisdUtilizacion = completeResultSet[i].getValue({
                            name: resultSearch.columns[19]
                        });//INDICA EL ID INTERNO DE LA JURISDICCION DE UTILIZACION A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].nombreJurisdUtilizacion = completeResultSet[i].getText({
                            name: resultSearch.columns[19]
                        });//INDICA EL NOMBRE DE LA JURISDICCION DE UTILIZACION A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].jurisdOrigen = completeResultSet[i].getValue({
                            name: resultSearch.columns[20]
                        });//INDICA EL ID INTERNO DE LA JURISDICCION DE ORIGEN A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].nombreJurisdOrigen = completeResultSet[i].getText({
                            name: resultSearch.columns[20]
                        });//INDICA EL NOMBRE DE LA JURISDICCION DE ORIGEN A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].jurisdiccionEntregaID = completeResultSet[i].getValue({
                            name: resultSearch.columns[21]
                        });//INDICA EL ID INTERNO DE LA JURISDICCION DE DESTINO A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].jurisdiccionEntregaNombre = completeResultSet[i].getText({
                            name: resultSearch.columns[21]
                        });//INDICA EL NOMBRE DE LA JURISDICCION DE DESTINO A NIVEL DE LÍNEA

                        informacionCodigosVBLineas[i].jurisdFacturacion = completeResultSet[i].getValue({
                            name: resultSearch.columns[22]
                ...
                log.debug('FIN - obtener_arreglo_codigos_ret_vendorbill_lineas', 'RETURN - informacionCodigosVBLineas: ' + JSON.stringify(informacionCodigosVBLineas));
                return informacionCodigosVBLineas;
            } catch (e) {
                log.error('obtener_arreglo_codigos_ret_vendorbill_lineas', 'obtener_arreglo_codigos_ret_vendorbill_lineas - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }

        function obtener_arreglo_netos_vendorbill_lineas(entidad, billsPagar, subsidiaria, sinJurisdiccionUnica, esONG) {

            try {
                log.debug('INICIO - obtener_arreglo_netos_vendorbill_lineas', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - sinJurisdiccionUnica: ' + sinJurisdiccionUnica + ' - esONG: ' + esONG);

                var informacionNetosVBLineas = [];

                var searchNetosVB = search.load({
                    id: 'customsearch_l54_bill_net_amt_lineas'
                });
                ...
                if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                    for (var i = 0; i < completeResultSet.length; i++) {
                        informacionNetosVBLineas[i] = new Object();

                        informacionNetosVBLineas[i].idInterno = completeResultSet[i].getValue({
                            name: resultSearch.columns[1]
                        });//ID INTERNO BILL

                        informacionNetosVBLineas[i].importe = parseFloat(completeResultSet[i].getValue({
                            name: resultSearch.columns[2]
                        }), 10).toFixedOK(2);//IMPORTE NETO

                        /* informacionNetosVBLineas[i].jurisdiccionEntregaID = completeResultSet[i].getValue({
                            name: resultSearch.columns[3]
                        });//L54 - Jurisdicción Entrega

                        informacionNetosVBLineas[i].jurisdiccionEntregaCodigo = completeResultSet[i].getValue({
                            name: resultSearch.columns[4]
                        });//L54 - Jurisdicción Entrega - Código Jurisdicción

                        informacionNetosVBLineas[i].jurisdiccionEntregaNombre = completeResultSet[i].getValue({
                            name: resultSearch.columns[5]
                        });//L54 - Jurisdicción Entrega - Nombre */

                        informacionNetosVBLineas[i].codRetencionGanancias = completeResultSet[i].getValue({
                            name: resultSearch.columns[6]
                        });//INDICA EL CÓDIGO DE LA RETENCIÓN DE GANANCIAS A NIVEL DE LÍNEA

                        informacionNetosVBLineas[i].nombreRetencionGanancias = completeResultSet[i].getText({
                            name: resultSearch.columns[6]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE GANANCIAS A NIVEL DE LÍNEA

                        informacionNetosVBLineas[i].codRetencionSuss = completeResultSet[i].getValue({
                            name: resultSearch.columns[7]
                        });//INDICA EL CÓDIGO DE LA RETENCIÓN DE SUSS A NIVEL DE LÍNEA

                        informacionNetosVBLineas[i].nombreRetencionSuss = completeResultSet[i].getText({
                            name: resultSearch.columns[7]
                        });//INDICA EL NOMBRE DE LA RETENCIÓN DE SUSS A NIVEL DE LÍNEA

                        informacionNetosVBLineas[i].lineNumber = completeResultSet[i].getValue({
                            name: resultSearch.columns[8]
                        });//INDICA EL NRO DE LÍNEA

                        informacionNetosVBLineas[i].importeBruto = parseFloat(completeResultSet[i].getValue({
                            name: resultSearch.columns[9]
                        }), 10).toFixedOK(2);//INDICA EL IMPORTE BRUTO DE LÍNEA



                        /********************************* DATOS JURISDICCION UTILIZACION *****************************/
                        
                        informacionNetosVBLineas[i].jurisdUtilizacion = completeResultSet[i].getValue({
                            name: resultSearch.columns[10]
                        });//INDICA EL ID INTERNO DE LA JURISDICCION DE UTILIZACION A NIVEL DE LÍNEA

                        informacionNetosVBLineas[i].nombreJurisdUtilizacion = completeResultSet[i].getText({
                            name: resultSearch.columns[10]
                        });//INDICA EL NOMBRE DE LA JURISDICCION DE UTILIZACION A NIVEL DE LÍNEA
                        ...
                        informacionNetosVBLineas[i].jurisdOrigen = completeResultSet[i].getValue({
                            name: resultSearch.columns[11]
                        });//INDICA EL ID INTERNO DE LA JURISDICCION DE ORIGEN A NIVEL DE LÍNEA

                        informacionNetosVBLineas[i].nombreJurisdOrigen = completeResultSet[i].getText({
                            name: resultSearch.columns[11]
                        });//INDICA EL NOMBRE DE LA JURISDICCION DE ORIGEN A NIVEL DE LÍNEA
                        ...
                        informacionNetosVBLineas[i].jurisdiccionEntregaNombre = completeResultSet[i].getText({
                            name: resultSearch.columns[12]
                        });//INDICA EL NOMBRE DE LA JURISDICCION DE DESTINO A NIVEL DE LÍNEA
                        ...
                        informacionNetosVBLineas[i].calcRetNoInscJurEnt = convertToBoolean(completeResultSet[i].getValue({
                            name: resultSearch.columns[14]
                        }));
                        ...
                        informacionNetosVBLineas[i].jurisdFacturacion = completeResultSet[i].getValue({
                            name: resultSearch.columns[15]
                        });

                        informacionNetosVBLineas[i].nombreJurisdFacturacion = completeResultSet[i].getText({
                            name: resultSearch.columns[15]
                        });
                ...
                // log.debug('obtener_arreglo_netos_vendorbill_lineas', 'RETURN - informacionNetosVBLineas: ' + JSON.stringify(informacionNetosVBLineas));
                log.debug('FIN - obtener_arreglo_netos_vendorbill_lineas', JSON.stringify(informacionNetosVBLineas) + ' ** TIEMPO' + new Date());
                return informacionNetosVBLineas;

            } catch (e) {
                log.error('obtener_arreglo_netos_vendorbill_lineas', 'obtener_arreglo_netos_vendorbill_lineas - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }

        function unificar_importes_lineas_facturas(resultsCodigosRetLineas, resultsNetosLineas) {

            try {
                log.debug('INICIO - unificar_importes_lineas_facturas', 'Parámetros: resultsCodigosRetLineas: ' + JSON.stringify(resultsCodigosRetLineas) + ' - resultsNetosLineas: ' + JSON.stringify(resultsNetosLineas));

                var arrayLineasFacturasUnificadas = [];

                if (!utilidades.isEmpty(resultsNetosLineas)) {
                    for (var i = 0; i < resultsNetosLineas.length; i++) {
                        var infoLinea = resultsCodigosRetLineas.filter(function (obj) {
                        ...
                        if (!utilidades.isEmpty(infoLinea) && infoLinea.length > 0) {
                            ...
                            objInfo.importeBruto = parseFloat(infoLinea[0].importeTotal, 10);
                            objInfo.importeNeto = parseFloat(infoLinea[0].importeNeto, 10);
                            objInfo.codRetencionGanancias = infoLinea[0].codRetencionGanancias;
                            objInfo.codRetencionSuss = infoLinea[0].codRetencionSuss;
                            ...
                            objInfo.jurisdUtilizacion = infoLinea[0].jurisdUtilizacion;
                            objInfo.nombreJurisdUtilizacion = infoLinea[0].nombreJurisdUtilizacion;
                            objInfo.jurisdOrigen = infoLinea[0].jurisdOrigen;
                            objInfo.nombreJurisdOrigen = infoLinea[0].nombreJurisdOrigen;
                            objInfo.jurisdiccionEntregaID = infoLinea[0].jurisdiccionEntregaID;
                ...
                log.debug('FIN - unificar_importes_lineas_facturas', 'RETURN - unificar_importes_lineas_facturas: ' + JSON.stringify(arrayLineasFacturasUnificadas));
                return arrayLineasFacturasUnificadas;

            } catch (e) {
                log.error('unificar_importes_lineas_facturas', 'unificar_importes_lineas_facturas - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }

        function unificar_info_facturas_lineas(arrayBillsPagadas, infoLineasFacturas) {

            try {
                log.debug('INICIO - unificar_info_facturas_lineas', 'Parámetros: arrayBillsPagadas: ' + JSON.stringify(arrayBillsPagadas) + ' - infoLineasFacturas: ' + JSON.stringify(infoLineasFacturas));

                var arrayBills = [];

                if (!utilidades.isEmpty(infoLineasFacturas) && infoLineasFacturas.length > 0) {
                    for (var i = 0; i < arrayBillsPagadas.length; i++) {
                        var infoFacturaPagar = infoLineasFacturas.filter(function (obj) {
                            return (arrayBillsPagadas[i].idVendorBill == obj.idInterno);
                        });

                        if (!isEmpty(infoFacturaPagar) && infoFacturaPagar.length > 0) {
                            for (var j = 0; j < infoFacturaPagar.length; j++) {
                                var objInfo = {};
                                objInfo.idVendorBill = arrayBillsPagadas[i].idVendorBill;
                                objInfo.amount = arrayBillsPagadas[i].amount;
                                objInfo.linea = arrayBillsPagadas[i].linea;
                                objInfo.lineNumber = infoFacturaPagar[j].lineNumber;
                                objInfo.importeBrutoLinea = infoFacturaPagar[j].importeBruto;
                                objInfo.importeNetoLinea = infoFacturaPagar[j].importeNeto;
                                objInfo.codRetencionGananciasLinea = infoFacturaPagar[j].codRetencionGanancias;
                                objInfo.codRetencionSussLinea = infoFacturaPagar[j].codRetencionSuss;
                                objInfo.nombreRetencionGanancias = infoFacturaPagar[j].nombreRetencionGanancias;
                                objInfo.nombreRetencionSuss = infoFacturaPagar[j].nombreRetencionSuss;
                                objInfo.esRetencionPorLinea = true;
                                objInfo.jurisdUtilizacion = infoFacturaPagar[j].jurisdUtilizacion;
                                objInfo.nombreJurisdUtilizacion = infoFacturaPagar[j].nombreJurisdUtilizacion;
                                objInfo.jurisdOrigen = infoFacturaPagar[j].jurisdOrigen;
                                objInfo.nombreJurisdOrigen = infoFacturaPagar[j].nombreJurisdOrigen;
                                objInfo.jurisdiccionEntregaID = infoFacturaPagar[j].jurisdiccionEntregaID;
                            ...
                            objInfo.nombreRetencionGanancias = '';
                            objInfo.nombreRetencionSuss = '';
                            objInfo.esRetencionPorLinea = false;
                            objInfo.jurisdUtilizacion = '';
                            objInfo.nombreJurisdUtilizacion = '';
                            objInfo.jurisdOrigen = '';
                            objInfo.nombreJurisdOrigen = '';
                            objInfo.jurisdiccionEntregaID = '';
                            objInfo.jurisdiccionEntregaNombre = '';
                            objInfo.jurisdFacturacion = '';
                            objInfo.nombreJurisdFacturacion = '';
                            arrayBills.push(objInfo);
                        }
                    }
                } else {
                ...
                log.debug('FIN - unificar_info_facturas_lineas', 'RETURN - arrayBills: ' + JSON.stringify(arrayBills));
                return arrayBills;

            } catch (e) {
                log.error('unificar_info_facturas_lineas', 'unificar_info_facturas_lineas - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }
        ...
        function getParametrosMatriz(arrayCodigosRetencion, esRetIIBB) {
            ...
            try {
                ...
                log.debug('getParametrosMatriz', 'INICIO - getParametrosMatriz');
                log.debug('getParametrosMatriz', 'Parámetros - arrayCodigosRetencion: ' + JSON.stringify(arrayCodigosRetencion) + ' - esRetIIBB: ' + esRetIIBB);
                var arrayParametrosMatriz = [];

                if (!isEmpty(arrayCodigosRetencion) && arrayCodigosRetencion.length > 0) {
                    //SAVE SEARCH A EJECUTAR
                    ...
                    if ((!isEmpty(completeResultSet)) && (completeResultSet.length > 0)) {
                        for (var i = 0; i < completeResultSet.length; i++) {
                            ...
                            objInfoDetaParametrizacion.monto_fijo_a_retener = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[0]
                            }), 10);//IMPORTE RETENIDO
                            ...
                            objInfoDetaParametrizacion.excedente = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[2]
                            }), 10);//EXCEDENTE

                            objInfoDetaParametrizacion.minimo_retencion = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[5]
                            }), 10);//IMPORTE MINIMO RETENCION

                            objInfoDetaParametrizacion.nombreRet = completeResultSet[i].getValue({
                                name: resultSearch.columns[6]
                            });//NOMBRE RETENCION

                            objInfoDetaParametrizacion.importeDesde = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[7]
                            }), 10);//IMPORTE DESDE

                            objInfoDetaParametrizacion.importeHasta = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[8]
                            }), 10);//IMPORTE HASTA

                            objInfoDetaParametrizacion.idCodigoRetencionPadre = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[9]
                            }), 10);//INTERNAL ID RETENCION PADRE

                            objInfoDetaParametrizacion.codigoRetencionPadre = parseFloat(completeResultSet[i].getValue({
                                name: resultSearch.columns[10]
                            }), 10);//RETENCION PADRE

                            arrayParametrosMatriz.push(objInfoDetaParametrizacion);
                        }
                    } else {
                        log.debug('getParametrosMatriz', 'getParametrosMatriz - No se encontró información de detalle de parametrización para el código de retención');
                    }
                }

                log.debug('getParametrosMatriz', 'RETURN - arrayParametrosMatriz: ' + JSON.stringify(arrayParametrosMatriz));
                log.debug('getParametrosMatriz', 'FIN - getParametrosMatriz');
                return arrayParametrosMatriz;
            } catch (e) {
                log.error('getParametrosMatriz', 'getParametrosMatriz - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }

        function getInfoRetencionCalcAnteriores(id_proveedor, id_periodo, codigo_retencion, subsidiaria, esAnualizada, fecha_pago) {
        ...
        function obtener_arreglo_codigos_ret_vendorbill_lineas_ong(entidad, billsPagar, subsidiaria, esONG) {

            try {
                log.debug('INICIO - obtener_arreglo_codigos_ret_vendorbill_lineas_ong', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - es ONG: ' + esONG);

                var informacionCodigosVBLineas = [];

                //SAVE SEARCH A EJECUTAR
                var searchCodRet = search.load({
                    id: 'customsearch_l54_bill_cod_ret_lineas_ong'
                });
                ...
                if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                    ...
                    log.debug('obtener_arreglo_codigos_ret_vendorbill_lineas_ong', 'obtener_arreglo_codigos_ret_vendorbill_lineas_ong - No se encuentro informacion para las bills recibidas por parametro');
                }

                log.debug('FIN - obtener_arreglo_codigos_ret_vendorbill_lineas_ong', 'RETURN - informacionCodigosVBLineas: ' + JSON.stringify(informacionCodigosVBLineas));
                return informacionCodigosVBLineas;
            } catch (e) {
                log.error('obtener_arreglo_codigos_ret_vendorbill_lineas_ong', 'obtener_arreglo_codigos_ret_vendorbill_lineas_ong - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }

        function obtener_arreglo_netos_vendorbill_lineas_ong(entidad, billsPagar, subsidiaria, sinJurisdiccionUnica, esONG) {

            try {
                log.debug('INICIO - obtener_arreglo_netos_vendorbill_lineas_ong', 'Parámetros - entidad: ' + entidad + ' - billsPagar: ' + billsPagar + ' - subsidiaria: ' + subsidiaria + ' - sinJurisdiccionUnica: ' + sinJurisdiccionUnica + ' - esONG: ' + esONG);

                var informacionNetosVBLineas = [];

                var searchNetosVB = search.load({
                    id: 'customsearch_l54_bill_net_amt_lineas_ong'
                });
                ...
                if (!isEmpty(completeResultSet) && completeResultSet.length > 0) {
                    ...
                    log.debug('obtener_arreglo_netos_vendorbill_lineas_ong', 'obtener_arreglo_netos_vendorbill_lineas_ong - No se encuentro informacion para las bills recibidas por parametro');
                }
                log.debug('FIN - obtener_arreglo_netos_vendorbill_lineas_ong' + JSON.stringify(informacionNetosVBLineas) + ' ** TIEMPO' + new Date());
                return informacionNetosVBLineas;

            } catch (e) {
                log.error('obtener_arreglo_netos_vendorbill_lineas_ong', 'obtener_arreglo_netos_vendorbill_lineas_ong - Error Netsuite Excepción - Error: ' + e.message);
                return null;
            }
        }
        ...
        return {
            afterVendorPaymentCreated: afterVendorPaymentCreated
        };
    });