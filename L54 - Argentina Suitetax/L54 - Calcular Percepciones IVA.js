...
define(
  [
   "L54/utilidades", "N/runtime", "N/search"
  ],
  function (utilidades, runtime, search) {
    /*global define log*/
    // migrado desde l54_proceso_pv_iva | braian azcune 25/4/2023

    /**
     * Esto era calcular_percepcion_ventas()
     * 
    ...
    function onRequest(context) {
      ...
      try {
        ...
        const param_codigo_IVA = currentScript.getParameter("custscript_l54_ca_per_iva_tip_per_iva2");
        const taxType = currentScript.getParameter("custscript_l54_ca_per_iva_tax_type");
        // var jurisdiccionCordoba = context.getSetting('SCRIPT', 'custscript_l54_cal_perc_vent_jur_cordoba');
        log.debug(proceso, "Unidades Disponibles :" + currentScript.getRemainingUsage());
        log.debug(proceso, "Codigo de retencion :" + param_codigo_IVA);
        ...
        if (!utilidades.isEmpty(infoTransaccion) && !utilidades.isEmpty(infoTransaccion.informacionTransaccion)) {
          ...
          if (!utilidades.isEmpty(informacionTransaccion)) {
            ...
            const calcularPercepciones = informacionTransaccion.calcularPercepciones;
            ...
            log.debug("calcular_percepcion_ventas", "LINE 381 - jurisdiccionEntrega: " + jurisdiccionEntrega + " - infoJurisdiccionDireccionEntrega: " + JSON.stringify(infoJurisdiccionDireccionEntrega));
            ...
            if (!utilidades.isEmpty(clienteTransaccion) && !utilidades.isEmpty(total)) {
              log.debug("Calcular Percepciones en Ventas FLAG 1", "ENTRE AL IF DE CLIENTEE");
              ...
              if (calcularPercepciones == true) {
                log.debug("Calcular Percepciones en Ventas FLAG 2", "ENTRE AL IF DE calcularPercepciones");
                let mensajesErrores = "";
                ...
                if (respuestaPercepciones.error == false) {
                  let indiceItems = 0;
                  log.debug("Calcular Percepciones en Ventas FLAG 4", "ENTRE AL IF DE RESPUESTAPERCECPCION ERROR");
                  ...
                  for (let i = 0; i < informacionTransaccion.informacion_articulos_iva.length; i++) {
                    try {
                      ...
                      if (superaMinBaseCalc) {
                        ...
                        log.debug("calcularPercepcionesVentas log", "porcentajeFinal: " + porcentajeFinal);
                        ...
                        if (!utilidades.isEmpty(porcentajeFinal) && !isNaN(porcentajeFinal) && porcentajeFinal > 0) {
                          log.debug("calcularPercepcionesVentas log", "Entre al iff correcto");
      ...
      log.debug("Calcular Percepciones IVA", "Respuesta/Salida:" + JSON.stringify(informacionRespuestaJSON));
    ...
    function obtenerArregloConfDetalle(idConfGeneral) {
      ...
      if (!utilidades.isEmpty(idConfGeneral)) {
        try {
          ...
          if (resultadosConfDetalle != null && resultadosConfDetalle.length > 0) {
            log.debug("obtenerArregloConfDetalle", "resultadosConfDetalle.length= " + resultadosConfDetalle.length);
      ...
      log.debug("obtenerArregloConfDetalle FIN", "Final ObtenerArregloconfDetalle ");
    ...
    function agruparMontos(arregloConfiguracionDetalle, informacion_articulos_iva, tipoContribuyente) {
      ...
      log.debug("informacion_articulos_iva Longitud", "Longitud:" + informacion_articulos_iva.length);
      ...
      log.debug("result Longitud", "Longitud:" + result.length);
      return result;
    }
