...
define(["N/runtime", "N/email", "N/file", "3K/utilities", "N/render", "N/search", "N/format", "N/url", "N/record", "./LIB - Busquedas.js", "N/task"], (runtime, email, file, utilities, render, search, format, url, record, libBusquedas, task) => {
    ...
    const getInputData = () => {
        ...
        try {
            ...
            const savedSearch = search.load({
                id: "customsearch_l593_ventas_ats_2"
            });
    ...
    const map = (context) => {
        ...
        try {
            ...
            objMap["tComp"] = result.values["GROUP(custbody_l593_tipo_comprobante)"][0].value;
            objMap["estabComp"] = result.values["GROUP(custbody_l593_establecimiento)"][0].value;
            objMap["perComp"] = result.values["GROUP(postingperiod)"][0].value;
            objMap["custComp"] = result.values["GROUP(customer.internalid)"][0].value;
            objMap["nComp"] = result.values["SUM(formulanumeric)"];
            objMap["tipoSS"] = 2;
            ...
            if (arrSummarize.data.ventas.length > 0 ) {
                ...
                if(tamanio%resultsNumber == 0){
                    var objParametros = {};
                        ...
                        objParametros.custscript_l593_ats_ventas_2_rellamado = Number(parametros.rellamado) + 10;
                    ...
                    var objParametros = {};
                        objParametros.custscript_l593_ats_ventas_3_subsidiaria = parametros.subsidiaria;
                        objParametros.custscript_l593_ats_ventas_3_periodo = parametros.periodo;
                        objParametros.custscript_l593_ats_ventas_3_usuario = idUsuario;
                        objParametros.custscript_l593_ats_ventas_3_regimen = parametros.regimenSemestral;
                        objParametros.custscript_l593_ats_ventas_3_archivos = parametros.archivos;
                        objParametros.custscript_l593_ats_ventas_3_xml = parametros.xml;

                    var redirec = task.create({
                        taskType: task.TaskType.MAP_REDUCE,
                        scriptId: 'customscript_l593_ats_ventas_3',
                        deploymentId: 'customdeploy_l593_ats_ventas_3',
                        params: objParametros
                    });
                    redirec.submit();
    ...
    const getParameters = () => {
        ...
        return {
            regimenSemestral: script.getParameter({ name: "custscript_l593_ats_ventas_2_regimen" }),
            subsidiaria: script.getParameter({ name: "custscript_l593_ats_ventas_2_subsidiaria" }),
            periodo: script.getParameter({ name: "custscript_l593_ats_ventas_2_periodo" }),
            folderATS: script.getParameter({ name: "custscript_l593_ats_ventas_2_folder" }),
            estadoOk: script.getParameter({ name: "custscript_l593_ats_ventas_2_estado_ok" }),
            estadoError: script.getParameter({ name: "custscript_l593_ats_ventas_2_error" }),
            usuarioId: script.getParameter({ name: "custscript_l593_ats_ventas_2_usuario" }),
            rellamado: script.getParameter({ name: "custscript_l593_ats_ventas_2_rellamado" }),
            archivos: script.getParameter({ name: "custscript_l593_ats_ventas_2_archivos" }),
            xml: script.getParameter({ name: "custscript_l593_ats_ventas_2_xml" })
        };
    };
