...
define(["N/runtime", "N/email", "N/file", "3K/utilities", "N/render", "N/search", "N/format", "N/url", "N/record", "./LIB - Busquedas.js", "N/task"], (runtime, email, file, utilities, render, search, format, url, record, libBusquedas, task) => {
    ...
    const getInputData = (context) => {
        ...
        try {
            ...
            const savedSearch = search.load({
                id: "customsearch_l593_anuladas_ats"
            });
    ...
    const map = (context) => {
        ...
        try {
            ...
            objMap["tipoComprobante"] = result.values["MAX(formulatext)"];
            objMap["establecimiento"] = result.values["MAX(formulatext)_1"];
            objMap["puntoEmision"] = result.values["MAX(formulatext)_2"];
            objMap["secuencialInicio"] = result.values["MAX(formulatext)_3"];
            objMap["secuencialFin"] = result.values["MAX(formulatext)_4"];
            objMap["autorizacion"] = result.values["MAX(formulatext)_5"];
    ...
    const summarize = (summary) => {
        ...
        try {
            ...
            summary.output.iterator().each(function(key, value){
                ...
                resultado.anuladas.push(obj);
                return true;
            });
            arrSummarize.data = resultado;
            log.audit("tamanio",JSON.stringify(arrSummarize));
            log.audit("tamanio",arrSummarize.data.anuladas.length);
            
            let tamanio =  arrSummarize.data.anuladas.length;
            let idUsuario = runtime.getCurrentUser().id;

            if (arrSummarize.data.anuladas.length > 0 ) {
                ...
                function saveTxt(objSummarize){
                    ...
                    log.debug("objRespuesta lengt",(objSummarize.data.anuladas).length)
                    ...
                    if(cantidadBytes < 10000000){
                        ...
                        let savedFile = file.create({
                            name: "ATS_Anuladas_auxiliar_"  + contador  + auxFecha,
                        ...
                        let resultado = objSummarize.data.anuladas,
                        ...
                        arrSummarize_1.data.anuladas = primeraMitad;
                        saveTxt(arrSummarize_1);
                        arrSummarize_2.data.anuladas = segundaMitad;
                ...
                if(tamanio%resultsNumber == 0){
                    var objParametros = {};
                        ...
                        objParametros.custscript_l593_anuladas_rellamado = parametros.rellamado + 10;
                    ...
                    var objParametros = {};
                        ...
                        log.debug("objParametros", JSON.stringify(objParametros))
                    var redirec = task.create({
                        taskType: task.TaskType.MAP_REDUCE,
    ...
    const getParameters = () => {
        ...
        return {
            regimenSemestral: script.getParameter({ name: "custscript_l593_ats_anuladas_regimen" }),
            subsidiaria: script.getParameter({ name: "custscript_l593_ats_anuladas_subsidiaria" }),
            periodo: script.getParameter({ name: "custscript_l593_ats_anuladas_periodo" }),
            folderATS: script.getParameter({ name: "custscript_l593_ats_anuladas_folder" }),
            estadoOk: script.getParameter({ name: "custscript_l593_ats_anuladas_estado_ok" }),
            estadoError: script.getParameter({ name: "custscript_l593_ats_anuladas_estado_erro" }),
            usuarioId: script.getParameter({ name: "custscript_l593_ats_anuladas_usuario" }),
            rellamado: script.getParameter({ name: "custscript_l593_ats_anuladas_rellamado" }),
            xml: script.getParameter({ name: "custscript_l593_ats_anuladas_xml" })
        };
    };
