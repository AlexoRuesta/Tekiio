    ...
    function (record, runtime, email, error, libSearch, utilities) {
        ...
        const getInputData = () => {
            ...
            try {
                ...
                let assetsExcludes= getAssetsExcludes(parameters.input.rangePeriod);
            ...
            try {
                ...
                if (!utilities.isEmpty(result)){
                    ...
                    if (parseFloat(indexCC, 10) < 0 || parseFloat(indexA, 10) < 0) {
                     ...
                     if (parseFloat(indexCC, 10) == 1 || parseFloat(indexA, 10) == 1) {
                        // Saltar esta entrada, sin hacer nada
                        return;
                    }
                    indexCC = parseFloat(indexCC, 10)/100 + 1;
                    indexA = parseFloat(indexA, 10)/100 + 1;
                    ...
                    if(!utilities.isEmpty(saveObjJournal)){
                        ...
                        for (let i = 1; i < indexPeriods.length; i++) {
                            ...
                            if((!getComparePeriods(dateDepreciation,current.enddate, ">") && !utilities.isEmpty(dateDepreciation)) || (getComparePeriods(purchasedate,current.startdate, "<") && !utilities.isEmpty(purchasedate)) ){
                                ...
                                index = Number(index) < 0 ? 1 : (parseFloat(index, 10)/100 + 1);
        ...
        const buildFilters = (parameters, assetsExcludes) => {
            ...
            if (!utilities.isEmpty(assetsExcludes) && assetsExcludes.length > 0 && !utilities.isEmpty(assetsExcludes[0]) ){
        ...
        const getAssetsExcludes = (rangePeriod) => {
            const idsPeriods = rangePeriod.map(obj => obj.internalid);
            let filters = [['custrecord_l56_audit_inflation_date_ini', 'anyof', idsPeriods]
                        ];
            let columns = [
                { name: 'formulatext', formula: 'NS_CONCAT({custrecord_l56_audit_inflation_asset.internalid})', alias: 'formulatext1' , summary: 'MIN'}
            ];

            let result = InitSearch.getSearchCreatedC('customrecord_l56_audit_inflation', filters, columns);
