...
define(["N/log", "N/url", "N/file", "N/encode", "N/runtime", "N/ui/serverWidget", "N/redirect", "N/task", "N/search", "LIB - Search", "LIB - Form", "L56/utilidades"], function (log, url, file, encode, runtime, serverWidget, redirect, task, search, libSearch, libForm, utilities) {
    ...
    const main = (context) => {
        try {
            ...
            if (method == "GET") {
                let nameReport = "CL - Correción Monetaria";
                ...
                userInterface.addSubmitButton("Generar Corrección Monetaria");
                ...
                if(context.request.parameters.custpage_period_init == context.request.parameters.custpage_period_end){
                    valorInit = rangePeriod.length > 0 ? rangePeriod[0].internalid : context.request.parameters.custpage_period_init;
                }else{
                    valorInit = context.request.parameters.custpage_period_init
                }
    ...
    report.excel = function (context){
        ...
        Object.keys(history).forEach(key => {
            ...
            for (let i = 0; i < history[key].length; i++) {
                ...
                let value1 = Math.round(savedSearch[col.col_0][0].col_14)
                xmlString += '<Row>';
                ...
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${value1}</Data></Cell>`; // Costo Original
                xmlString += `<Cell ss:StyleID="normal"><Data ss:Type="Number">${col.col_8}</Data></Cell>`; // Indice
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(costoActualInicial)}</Data></Cell>`; // Costo Actualizado al Inicio
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(col.col_7)}</Data></Cell>`; // Ajuste Sobre Costo
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(costoActualAcumulado)}</Data></Cell>`; // Costo Actualizado
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(amortizacion)}</Data></Cell>`; // Amortizacion Acumulada al Inicio
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(col.col_12)}</Data></Cell>`; // Ajuste Sobre Amortizacion Acumulada
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(amortizacionAcumulada)}</Data></Cell>`; // Amortizacion Acumulada Actualizada
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(col.col_11)}</Data></Cell>`; // Valor en Libros al Inicio
                xmlString += `<Cell ss:StyleID="Miles"><Data ss:Type="Number">${Math.round(libroActualizado)}</Data></Cell>`; // Valor en Libros Actualizado
    ...
    const printStyleExcel = () => {
         let strExcel = '';
            ...
            strExcel += '<Style ss:ID="Miles">';
            strExcel += '<Alignment ss:Vertical="Center"/>';
            strExcel += '<Interior ss:Color="#EAF1FB" ss:Pattern="Solid"/>';
            strExcel += '<NumberFormat ss:Format="#,##0.00"/>';
            strExcel += '<Borders>';
            strExcel += '<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#CCCCCC"/>';
            strExcel += '<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#CCCCCC"/>';
            strExcel += '<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#CCCCCC"/>';
