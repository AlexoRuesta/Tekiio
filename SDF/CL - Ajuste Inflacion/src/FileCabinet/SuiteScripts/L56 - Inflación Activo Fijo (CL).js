 ...
 *@NModuleScope Public
*/
define(['N/url', 'N/currentRecord', 'N/ui/dialog', 'N/search', 'N/record', 'N/runtime'], function (url, currentRecord, dialog, search, record, runtime) {
    ...
    const getSuiteletURL = () => {
        return url.resolveScript({
            scriptId: 'customscript_l56_inflation_asset_ind_sl',
            deploymentId: 'customdeploy_l56_inflation_asset_ind_sl',
    ...
    const getFiltersValue = (currentRecord) => {
        let values = {
            year: currentRecord.getValue('custpage_year')
        };
        return values;
    }
    ...
    function saveRecord(scriptContext) { 
        ...
        var origen = currentRecord.getValue({ fieldId:'custpage_origen'});

        console.log('origen ' + origen)
        if (origen == 'sl_principal') {
            let configuration = getConfiguration(currentRecord.getValue({ fieldId: "custpage_subsidiary" }));
    ...
    const deleteRecords = (journal, asset) => {
        let customRecord = "customrecord_l56_audit_inflation";
        var messague  = {
            title: "Atencion",
        ...
        function success(result) {
            ...
            if (result == true) {
                let customSearch = search.create({
                    type: customRecord,
                    filters:[
                                ["custrecord_l56_audit_inflation_journal","anyof", journal]
                            ],
                    columns: [
                        search.createColumn({
                            name: "internalid",
                        ...
                        search.createColumn({
                            name: "custrecord_l56_audit_inflation_ca",
                            summary: "GROUP",
                            label: "Costo Actual Historico"
                        }),
                        search.createColumn({
                            name: "custrecord_l56_audit_inflation_nbv",
                            summary: "GROUP",
                            label: "Valor en Libros Historico"
                        }),
                        search.createColumn({
                            name: "custbody_l56_altdepreciation",
                            join: "CUSTRECORD_L56_AUDIT_INFLATION_JOURNAL",
                            summary: "GROUP",
                            label: "CL - Depreciación Alternativa Asociada"
                        })
                        ]
                })
                ...
                if (customResult.length != 0) {
                    for (var i in customResult) {
                        ...
                        var currentCost = customResult[i].getValue({ name: "custrecord_l56_audit_inflation_ca", summary: "GROUP" });
                        var bookValue = customResult[i].getValue({ name: "custrecord_l56_audit_inflation_nbv", summary: "GROUP" });
                        var depreciation = customResult[i].getValue({ name: "custbody_l56_altdepreciation", join: "custrecord_l56_audit_inflation_journal", summary: "GROUP"});
                    }
                }
