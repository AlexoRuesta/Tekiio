    ...
    (record, error, search, utilities, runtime, format) => {
        ...
        function crearNumeradorAnul(recordRet , recordAnul){
            try {
                ...
                if (tipoRetencion == currScript.getParameter('custscript_l54_crea_anul_ret_inym'))//INYM
                {
                    var tipoTransIdINYM = currScript.getParameter('custscript_l54_crea_anul_ret_inym_anul');
                    numeradorArray = devolverNuevoNumero(tipoTransIdINYM, bocaId, letraId, subsidiaria, jurisdiccion);
                }
